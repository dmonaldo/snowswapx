<template>
  <div class="window white">
    <fieldset class="contractsdialog">
      <legend>
        <button @click="showContracts = 1" class="buttoncontracts">
          Contracts
        </button>
      ...
      <p class="simple-error">
        Do
        <b>not</b>
        send funds directly to the Curve contracts. You need to deposit them!
      </p>
      <div v-show="showContracts == 1">
        <fieldset>
          ...
          >
            <img
              class="icon"
              :src="publicPath + 'curveIcons/curve-registry.svg'"
            />
            <span class="text">Curve Registry address</span>
          </a>
          <a
          ...
          >
            <img
              ...
              :src="publicPath + 'curveIcons/curve-registry.svg'"
            />
            <span class="text">Curve calc address</span>
          </a>
          <a
        ...
        <fieldset>
          ...
          <a href="https://app.nexusmutual.io/#/SmartContractCover">
            <img
              class="icon"
              :src="publicPath + 'curveIcons/nexusmutual.png'"
            />
            <span class="text">Stablecoin pools - curvev2.nexusmutual.eth</span>
          </a>
          <a
            href="https://etherscan.io/address/0xc1DB00a8E5Ef7bfa476395cdbcc98235477cDE4E"
          >
            <img
              ...
              :src="publicPath + 'curveIcons/nexusmutual.png'"
            />
            <span class="text">BTC pools - curvebtc.nexusmutual.eth</span>
          </a>
        </fieldset>
        <fieldset v-for="(addresses, i) in contractAddresses">
          ...
          <a :href="'https://etherscan.io/address/' + addresses.swap">
            ...
            <span class="text">swap address</span>
          </a>
          <a :href="'https://etherscan.io/address/' + addresses.token">
            <img class="icon" :src="getTokenUrl(i)" />
            <span class="text">
              [{{ tokenNames[i].ticker }}] {{ tokenNames[i].name }} token
              address
            </span>
          </a>
          <a
            :href="'https://etherscan.io/address/' + depositZaps[i].deposit"
            v-show="!['ren', 'sbtc'].includes(allPools[i])"
          >
            <img class="icon" :src="getTokenUrl(i)" />
            <span class="text">deposit address</span>
          </a>
          <a
            :href="
              'https://etherscan.io/address/' + rewardsAddresses[allPools[i]]
            "
            v-show="['susdv2', 'sbtc', 'y', 'iearn'].includes(allPools[i])"
          >
            <img class="icon" :src="getTokenUrl(i)" />
            <span class="text">staking rewards address</span>
          </a>
          <a
            href="https://etherscan.io/address/0x9fe350DfA5F66bC086243F21A8F0932514316627"
            v-show="['ren'].includes(allPools[i])"
          >
            <img class="icon" :src="getTokenUrl(i)" />
            <span class="text">old adapter address</span>
          </a>
          <a
            :href="
              'https://etherscan.io/address/' + adapterAddresses[allPools[i]]
            "
            v-show="['ren', 'sbtc'].includes(allPools[i])"
          >
            <img class="icon" :src="getTokenUrl(i)" />
            <span class="text">adapter address</span>
          </a>
          <a
            href="https://etherscan.io/token/0x0bc529c00c6401aef6d220be8c6ea1667f6ad93e"
            v-show="['y', 'iearn'].includes(allPools[i])"
          >
      ...
      <div v-show="showContracts == 3">
        <fieldset>
          ...
          >
            <img class="icon" :src="publicPath + 'logo.png'" />
            <span class="text">CRV token</span>
          </a>
          <a
        ...
        <fieldset>
          ...
          <a href="https://mainnet.aragon.org/#/dao.curvefi.eth">
            <img class="icon" :src="publicPath + 'curveIcons/aragon.png'" />
            <span class="text">Aragon DAO</span>
          </a>
          <a href="https://mainnet.aragon.org/#/curveemergency">
            <img class="icon" :src="publicPath + 'curveIcons/aragon.png'" />
            <span class="text">Emergency DAO</span>
          </a>
        </fieldset>
        <fieldset v-for="(addresses, i) in gaugeAddresses">
          ...
          <a
            :href="'https://etherscan.io/address/' + addresses.address"
            :class="{ disabledGauge: allPools[i] == 'usdt' }"
          >
            <img class="icon" :src="getTokenUrl(i)" />
            <span class="text">{{ allPools[i] }} Gauge address</span>
          </a>
        </fieldset>
      </div>
    </fieldset>
...
export default {
  ...
  computed: {
    contractAddresses() {
      return Object.keys(allabis)
        .filter((pool) => !["y", "susd", "tbtc", "hbtc"].includes(pool))
        .map((pool) => ({
          swap: allabis[pool].swap_address,
          token: allabis[pool].token_address,
        }));
    },
    depositZaps() {
      return Object.keys(allabis)
        .filter((pool) => !["y", "susd", "tbtc", "hbtc"].includes(pool))
        .map((pool) => ({
          deposit: allabis[pool].deposit_address,
          token: allabis[pool].token_address,
        }));
    },
    publicPath() {
  ...
  metaInfo: {
    title: "snowswap.org :: Contracts",
    meta: [
      { property: "og:title", content: "beta.snowswap.org/contracts" },
      { property: "og:url", content: "https://snowswap.org/contracts" },
      ...
      { name: "twitter:title", content: "beta.snowswap.org/contracts" },
      ...
      { name: "twitter:url", content: "https://snowswap.org/contracts" },
