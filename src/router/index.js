...
const WithdrawRouter = () =>
  import("../components/withdraw/WithdrawRouter.vue");
...
let routes = [
  ...
  {
    path: "/:pool(compound|usdt|y|iearn|busd|susdv2|pax|tbtc|ren|sbtc|hbtc)/",
    name: "Index",
    component: PoolApp,
    children: [
      {
...
router.beforeEach(async (to, from, next) => {
  if (from.path.includes("/compound/withdraw_old"))
  ...
  if (!pools.includes(subdomain)) subdomain = contract.currentContract;
  ...
  if (
    (currentContract.currentContract != subdomain &&
      !["Stats", "FAQ", "Donate"].includes(to.name)) ||
    ["Stats", "FAQ", "Donate"].includes(from.name)
  ) {
