...
export function approve(contract, amount, account, toContract) {
  ...
  return new Promise((resolve, reject) => {
    contract.methods
      .approve(toContract, cBN(amount).toFixed(0, 1))
      .send({
        from: account,
...
export function approve_to_migrate(amount, account) {
  return new Promise((resolve) => {
    currentContract.old_swap_token.methods
      .approve(currentContract.migration_address, amount)
      .send({
        from: account,
        gasPrice: gasPriceStore.state.gasPriceWei,
        gas: 100000,
...
export async function ensure_allowance_zap_out(
  amount,
  fromContract,
  toContract,
  infinite = false
  ...
  if (!toContract)
    toContract = allabis[currentContract.currentContract].deposit_address;
  let allowance = cBN(
    await currentContract.swap_token.methods
      .allowance(default_account, toContract)
      .call()
  );
  if (!infinite) {
    ...
    if (
      allowance.lt(currentContract.max_allowance.div(cBN(2))) &&
      cBN(amount).gt(0)
    ) {
      if (
        allowance > 0 &&
        requiresResetAllowance.includes(fromContract._address)
      )
        await approve(fromContract, 0, default_account, toContract);
      await approve(
        fromContract,
...
export async function ensure_allowance(
  amounts,
  plain = false,
  contractName,
  N_COINS,
  ...
  for (let i = 0; i < N_COINS; i++) {
    calls.push([
      coins[i]._address,
      coins[i].methods.allowance(default_account, swap).encodeABI(),
    ]);
  }
  let aggcalls = await currentContract.multicall.methods
    .aggregate(calls)
    .call();
  allowances = aggcalls[1].map((hex) =>
  ...
  if (!infinite) {
    ...
    for (let i = 0; i < N_COINS; i++) {
      if (
        cBN(allowances[i]).isLessThan(cBN(amounts[i])) &&
        cBN(amounts[i]).gt(0)
      ) {
        if (
          allowances[i] > 0 &&
          requiresResetAllowance.includes(coins[i]._address)
        )
    ...
    for (let i = 0; i < N_COINS; i++) {
      if (
        cBN(allowances[i]).isLessThan(cont.max_allowance.div(cBN(2))) &&
        cBN(amounts[i]).gt(0)
      ) {
        if (
          allowances[i] > 0 &&
...
export async function ensure_underlying_allowance(
  i,
  _amount,
  underlying_coins = [],
  toContract,
  ...
  var current_allowance = cBN(
    await coins[i].methods
      .allowance(default_account, contract.swap._address)
      .call()
  );
  if (current_allowance.gte(amount)) return false;
  if (
    cBN(_amount).isEqualTo(currentContract.max_allowance) &
    current_allowance.isGreaterThan(currentContract.max_allowance.div(cBN(2)))
  )
    return false; // It does get spent slowly, but that's ok
  if (
    current_allowance.isGreaterThan(cBN(0)) &
    current_allowance.isLessThan(amount) &&
    requiresResetAllowance.includes(coins[i]._address)
  )
...
export async function approveAmount(
  ...
  let current_allowance = cBN(
    await contract.methods.allowance(account, toContract).call()
  );
  console.log(currentContract.max_allowance);
  console.log(
    current_allowance.toString(),
    amount.toString(),
    current_allowance.lt(amount)
  );
  if (!infinite) {
    if (current_allowance.lt(amount) && amount.gt(0)) {
      if (
        current_allowance > 0 &&
        requiresResetAllowance.includes(contract._address)
      ) {
    ...
    if (
      current_allowance.lt(currentContract.max_allowance.div(cBN(2))) &&
      cBN(amount).gt(0)
    ) {
      if (
        current_allowance > 0 &&
...
export async function ensure_token_allowance() {
  ...
  if (
    parseInt(
      await currentContract.swap_token.methods
        .allowance(default_account, currentContract.swap_address)
        .call()
    ) == 0
  )
    return new Promise((resolve) => {
      currentContract.swap_token.methods
        .approve(
          ...
          cBN(currentContract.max_allowance).toFixed(0)
        )
...
export function init_menu() {
  $("div.top-menu-bar a")
    .toArray()
    .forEach(function (el) {
      if (el.href == window.location.href) el.classList.add("selected");
    });
  $(".poolsdropdown .dropdown a")
    .toArray()
    .forEach(function (el) {
...
export async function ensure_stake_allowance(
  amount,
  stakeContract,
  infinite = false
) {
  ...
  let allowance = cBN(
    await currentContract.swap_token.methods
      .allowance(default_account, stakeContract._address)
      .call()
  );
  if (!infinite) {
    ...
    if (
      ...
      if (
        allowance > 0 &&
        requiresResetAllowance.includes(currentContract.swap_token._address)
      )
        await approve(
          currentContract.swap_token,
          0,
  ...
  for (let i = 0; i < allabis[contract.currentContract].N_COINS; i++) {
    ...
    if (checkTethered(contract, i)) {
      Vue.set(
        contract.c_rates,
        i,
        1 / allabis[contract.currentContract].coin_precisions[i]
      );
    } else if (
      ["iearn", "busd", "susd", "pax"].includes(contract.currentContract)
    ) {
      ...
      else calls.push([address, "0x77c7b8fc"]);
...
export async function update_fee_info(
  version = "new",
  contract,
  update = true
) {
  ...
  var default_account =
    contract.default_account || "0x0000000000000000000000000000000000000000";
  let calls = [
    //.fee()
    ...
    [
      swap_token_address,
      swap_token_stats.methods.balanceOf(default_account).encodeABI(),
    ],
    //token_supply()
  ...
  calls.push([
    allabis[contract.currentContract].gauge_address,
    contract.gaugeContract.methods.balanceOf(default_account).encodeABI(),
  ]);
  if (update) await multiInitState(calls, contract);
  return calls;
...
function checkTethered(contract, i) {
  return (
    (allabis[contract.currentContract].tethered &&
...
export async function multiInitState(calls, contract, initContracts = false) {
  ...
  try {
    ...
    aggcalls[1] = [
      web3.eth.abi.encodeParameter("uint256", cBN(1e18).toFixed(0)),
      ...aggcalls[1],
    ];
  }
  ...
  if (
    initContracts &&
    ["sbtc", "iearn", "y"].includes(contract.currentContract)
  ) {
    //decoded = decoded.slice(1);
  }
  if (initContracts) {
  ...
  if (
    initContracts &&
    ["tbtc", "ren", "sbtc", "hbtc"].includes(contract.currentContract)
  ) {
  ...
  let ratesDecoded = decoded.slice(
    4 + allabis[contract.currentContract].N_COINS
  );
  if (initContracts) {
    let contractsDecoded = decoded.slice(
      -allabis[contract.currentContract].N_COINS * 2
    );
    chunkArr(contractsDecoded, 2).map((v, i) => {
      ...
      if (
        (contract.currentContract == "susdv2" && i == 3) ||
        (contract.currentContract == "sbtc" && i == 2)
      ) {
      ...
      contract.underlying_coins.push(
        new web3.eth.Contract(underlying_abi, underlying_addr)
      );
    });
    window[contract.currentContract].coins = contract.coins;
    window[contract.currentContract].underlying_coins =
      contract.underlying_coins;
    ratesDecoded = decoded.slice(
      4 + allabis[contract.currentContract].N_COINS,
      decoded.length - allabis[contract.currentContract].N_COINS * 2
    );
  }
  ...
  if (["iearn", "busd", "susd", "pax"].includes(contract.currentContract)) {
    ratesDecoded.map((v, i) => {
      if (checkTethered(contract, i)) {
        ...
        let rate =
          v / 1e18 / allabis[contract.currentContract].coin_precisions[i];
    ...
    chunkArr(ratesDecoded, 3).map((v, i) => {
      if (checkTethered(contract, i) || contract.currentContract == "susdv2") {
        ...
        let rate =
          +v[0] / 1e18 / allabis[contract.currentContract].coin_precisions[i];
        let supply_rate = +v[1];
        let old_block = +v[2];
        Vue.set(
          contract.c_rates,
          i,
          rate * (1 + (supply_rate * (block - old_block)) / 1e18)
        );
      }
    });
  }
  ...
  let balancesDecoded = decoded.slice(
    4,
    4 + allabis[contract.currentContract].N_COINS
  );
  ...
  if (!initContracts) contract.curveStakedBalance = decoded[decoded.length - 1];
  ...
  if (default_account) {
    if (token_balance > 0) {
      ...
      for (let i = 0; i < contract.N_COINS; i++) {
        var val =
          (balances[i] * contract.c_rates[i] * token_balance) / token_supply;
    ...
    if (contract.curveStakedBalance > 0) {
      for (let i = 0; i < contract.N_COINS; i++) {
        var val =
          (balances[i] * contract.c_rates[i] * contract.curveStakedBalance) /
          token_supply;
        Vue.set(contract.staked_info, i, val);
        contract.totalStake += val;
      }
      contract.usdStake =
        (contract.curveStakedBalance * contract.virtual_price) / 1e18;
    }
  }
}
...
export async function handle_migrate_new(page) {
  var default_account = currentContract.default_account;
  let migration = new web3.eth.Contract(
    allabis.compound.migration_abi,
    currentContract.migration_address
  );
  let old_balance = await currentContract.old_swap_token.methods
    .balanceOf(default_account)
    .call();
  var allowance = parseInt(
    await currentContract.old_swap_token.methods
      .allowance(default_account, currentContract.migration_address)
      .call()
  );
  if (allowance < old_balance) {
    if (allowance > 0) await approve_to_migrate(0, default_account);
    await approve_to_migrate(old_balance, default_account);
  }
  await migration.methods
    .migrate()
    .send({
...
export async function calc_slippage(values, deposit, zap_values, to_currency) {
  ...
  let calls = [
    [
      ...
      currentContract.swap.methods.get_virtual_price().encodeABI(),
    ],
  ];
  if (to_currency !== undefined) {
    let precision =
      allabis[currentContract.currentContract].coin_precisions[to_currency];
    ...
    zap_values[to_currency] = zap_values[to_currency]
      .div(1e18)
      .div(currentContract.c_rates[to_currency])
      .toFixed(0);
    calls.push([
      currentContract.swap._address,
      currentContract.swap.methods
        .calc_token_amount(zap_values, to_currency)
        .encodeABI(),
    ]);
  } else {
    ...
    var values = real_values.map((x, i) =>
      cBN(Math.floor(x / currentContract.c_rates[i]).toString()).toFixed(0, 1)
    );
    calls.push([
      currentContract.swap._address,
      currentContract.swap.methods
        .calc_token_amount(values, deposit)
        .encodeABI(),
    ]);
  }
  calls.push(
    ...[...Array(currentContract.N_COINS).keys()].map((i) => [
      currentContract.swap._address,
  ...
  for (let i = 0; i < currentContract.N_COINS; i++) {
    ...
    if (!deposit) {
      ...
      } else currentContract.showNoBalance = false;
    }
  }
  if (deposit) slippage = Sv / Sr;
  else if (to_currency === undefined) {
    slippage = Sr / Sv;
  } else slippage = Sr / (Sv * 1e18);
  slippage = slippage - 1;
  slippage = slippage || 0;
