...
let calc = ({
  ...
  function get_D(xp) {
    ...
    for (let _i = 0; _i < 255; _i++) {
      ...
      D = d4.div(d7);
      if (D.gt(Dprev))
        if (D.minus(Dprev).lte(BN(1))) break;
        else if (Dprev.minus(D).lte(BN(1))) break;
    }
    return D;
  }
  ...
  function get_y(i, j, x, _xp) {
    ...
    for (let _i = 0; _i < N_COINS; _i++) {
      ...
      S_ = S_.plus(_x);
      c = c.times(D).div(_x.times(BN(N_COINS)));
    }
    c = c.times(D).div(Ann.times(N_COINS));
    ...
    for (let _i = 0; _i < 255; _i++) {
      ...
      if (y.gt(y_prev))
        if (y.minus(y_prev).lte(BN(1))) break;
        else if (y_prev.minus(y).lte(BN(1))) break;
    }
    return y;
  }
