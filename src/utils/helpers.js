...
export function getTokenIcon(token, wrapped, pool) {
  if (wrapped && ["compound", "usdt"].includes(pool) && token != "pax") {
    ...
    wrapped &&
    ["iearn", "y", "busd", "pax"].includes(pool) &&
    token != "pax"
  ) {
    token = "_y" + token;
  }
  ...
  let asset;
  let pngs = ["dai", "busd", "pax", "hbtc", "_ybcrv", "ycrv", "bcrv"];
...
export async function getETHPrice() {
  ...
  try {
    let req = await Promise.race([
      fetch("https://pushservice.snowswap.org/getETHprice"),
      fetch(
...
export function interp(x, y, val) {
  ...
  return (val) => {
    if ((val <= x_min) | (val >= x_max)) return 0;
...
export function chunkArr(arr, chunks) {
  return new Array(Math.ceil(arr.length / chunks))
    .fill()
    .map((_, i) => arr.slice(i * chunks, i * chunks + chunks));
}
...
export async function AES_GCM_encrypt(message, secretkey) {
  ...
  let key = await crypto.subtle.importKey("raw", keyData, "aes-gcm", false, [
    "encrypt",
  ]);
...
export async function AES_GCM_decrypt(message, secretkey) {
  ...
  let plainText = await crypto.subtle.decrypt(
    ...
    key,
    hexStringToUint8Array(cipherHex)
  );
...
export function asciiToUint8Array(str) {
  var chars = [];
  for (var i = 0; i < str.length; ++i) chars.push(str.charCodeAt(i));
