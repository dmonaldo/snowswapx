...
export const gas = {
  swap: {
    ...
    iearn: {
      ...
      exchange_underlying: (i, j) => 2000000,
    },
    busd: {
      exchange: (i, j) => 1600000,
      exchange_underlying: (i, j) => 2000000,
  ...
  withdraw: {
    ...
    iearn: {
      imbalance: (x) => Math.max(
        ((12642 * x + 1074068) * 2.5) | 0,
        1600000
      )
    },
    busd: {
  ...
  depositzap: {
    ...
    //who the fuck came up with these formulae and how??
    iearn: {
      deposit: (x) => ((225377 * x + 522674) * 2.5) | 0, // original * 2
      withdraw: 3500000 / 1.1, //original value set at 1.4
...
export async function init(contract, refresh = false) {
  state.multicall =
    state.multicall || new web3.eth.Contract(multicall_abi, multicall_address);
  console.time("init");
  //contract = contracts.compound for example
  if (
    state.initializedContracts &&
  ...
  if (
    contract &&
    (contract.currentContract == state.currentContract ||
      state.contracts[contract.currentContract].initializedContracts) &&
    !refresh
  )
    return Promise.resolve();
  ...
  contract.gaugeContract = new state.web3.eth.Contract(
    liquiditygauge_ABI,
    allabis[contract.currentContract].gauge_address
  );

  contract.snowGeyser = new state.web3.eth.Contract(geyser_abi, geyser_snow_eth)
  contract.ySnowGeyser = new state.web3.eth.Contract(geyser_abi, geyser_ysnow_snow)
  contract.ycrvSnowGeyser = new state.web3.eth.Contract(geyser_abi, geyser_ycrvsnow_snow)
  contract.votingToken = new state.web3.eth.Contract(ERC20_abi, snow_address)
  ...
  if (contract.currentContract == "compound") {
    state.old_swap = new state.web3.eth.Contract(
      allabis.compound.old_swap_abi,
      old_swap_address
    );
    state.old_swap_token = new state.web3.eth.Contract(
      ERC20_abi,
      old_token_address
    );
    calls.push([
      state.old_swap_token._address,
      state.old_swap_token.methods
        .balanceOf(
          state.default_account || "0x0000000000000000000000000000000000000000"
        )
        .encodeABI(),
  ...
  if (["ren", "sbtc"].includes(contract.currentContract)) {
    state.adapterContract = new state.web3.eth.Contract(
      allabis[contract.currentContract].adapterABI,
      allabis[contract.currentContract].adapterBiconomyAddress
    );
  }
  if (contract.currentContract == "susdv2") {
    ...
    let default_account =
      state.default_account || "0x0000000000000000000000000000000000000000";
    calls.push([
      allabis.susd.token_address,
      "0x70a08231000000000000000000000000" + default_account.slice(2),
    ]);

    contract.curveRewards = new state.web3.eth.Contract(
    ...
    contract.snxExchanger = new state.web3.eth.Contract(
      synthetixExchanger_ABI,
      synthetixExchanger_address
    );
  }
  ...
  if (["iearn", "y"].includes(contract.currentContract)) {
    contract.aRewards = new state.web3.eth.Contract(
      allabis.iearn.aRewards_abi,
      allabis.iearn.aRewards_address
    );
    contract.curveRewards = new state.web3.eth.Contract(
  ...
  calls.push([
    contract.gaugeContract._address,
    contract.gaugeContract.methods
      .balanceOf(
        state.default_account || "0x0000000000000000000000000000000000000000"
      )
      .encodeABI(),
  ...
  if (
    !["susd", "tbtc", "ren", "sbtc", "hbtc"].includes(contract.currentContract)
  )
    state.deposit_zap = new state.web3.eth.Contract(
      allabis[state.currentContract].deposit_abi,
      allabis[state.currentContract].deposit_address
    );
  contract.swap = new state.web3.eth.Contract(
    allabis[contract.currentContract].swap_abi,
    allabis[contract.currentContract].swap_address
  );
  contract.swap_token = new state.web3.eth.Contract(
    ERC20_abi,
    allabis[contract.currentContract].token_address
  );
  ...
  window[contract.currentContract].snowGeyser = contract.snowGeyser
  window[contract.currentContract].ySnowGeyser = contract.ySnowGeyser
  window[contract.currentContract].ycrvSnowGeyser = contract.ycrvSnowGeyser
  window[contract.currentContract].votingToken = contract.votingToken
  ...
  else calls.push(...(await common.update_fee_info("new", contract, false)));
  for (let i = 0; i < allabis[contract.currentContract].N_COINS; i++) {
    let coinsCall = contract.swap.methods.coins(i).encodeABI();
    let underlyingCoinsCall = ["tbtc", "ren", "sbtc", "hbtc"].includes(
  ...
  state.allInitContracts = new Set(
    state.allInitContracts.add(contract.currentContract)
  );
...
export async function getAllUnderlying() {
  for ([key, contract] of Object.entries(allabis)) {
    ...
    allState.swap[key] = new state.web3.eth.Contract(
      contract.swap_abi,
    ...
    for (let i = 0; i < contract.N_COINS; i++) {
      ...
      var underlying_addr = await allState.swap[key].swap.methods
        .underlying_coins(i)
        .call();
      allState.underlying_coins[key][i] = new state.web3.eth.Contract(
        ERC20_abi,
        underlying_addr
      );
    }
