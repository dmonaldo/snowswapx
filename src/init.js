...
if (isMobile && !window.web3) {
  wallets = wallets.filter(
    (wallet) =>
      !["trust", "imToken", "status", "coinbase"].includes(wallet.walletName)
  );
}
    ...
    address: (account) => {
      if (account === undefined) {
        ...
        if (
          ...
          state.contract.initializedContracts
        )
...
async function init(init = true, name, walletlink = false) {
  ...
  try {
    ...
    state.contract.multicall = new state.contract.web3.eth.Contract(
      multicall_abi,
      multicall_address
    );
