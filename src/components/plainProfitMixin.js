...
export default {
  ...
  async created() {
    this.$watch(
      () => this.account && currentContract.multicall,
      (val) => val && this.getPrices()
    );
  },
  ...
  computed: {
    ...
    showWeeklySNX() {
      if (this.showinUSD == 1)
        return (+this.weeklyEstimateSNX * this.snxPrice).toFixed(3);
      return (+this.weeklyEstimateSNX).toFixed(2);
    },

    showWeeklyREN() {
      if (this.showinUSD == 1)
        return (+this.weeklyEstimateREN * this.snxPrice).toFixed(3);
      return (+this.weeklyEstimateREN).toFixed(2);
    },

    showWeeklyBPT() {
      if (this.showinUSD == 1)
        return (+this.weeklyEstimateBPT * this.BPTPrice).toFixed(2);
  ...
  methods: {
    ...
    async getSNXRewards() {
      let requests = await Promise.all([
        fetch(
          "https://api.coingecko.com/api/v3/simple/price?ids=havven,republic-protocol&vs_currencies=usd"
        ),
      ]);
      ...
      let calls = [
        [
          ...
          curveRewards.methods.earned(this.account).encodeABI(),
        ],
        [
          curveRewards._address,
          curveRewards.methods.balanceOf(this.account).encodeABI(),
        ],
        [
          curveRewards._address,
          curveRewards.methods.userRewardPerTokenPaid(this.account).encodeABI(),
        ],
        [curveRewards._address, curveRewards.methods.totalSupply().encodeABI()],
      ];
      if (currentContract.currentContract == "sbtc") {
        let balancerPool = new currentContract.web3.eth.Contract(
          balancer_ABI,
          balancer_address
        );
        calls.push(
          [
            ...
            balancerPool.methods.totalSupply().encodeABI(),
          ],
          [
            balancerPool._address,
      ...
      if (currentContract.currentContract == "susdv2") {
        ...
        let rewards = rewardLogs
          .map(
            (log) =>
              currentContract.web3.eth.abi.decodeParameter(
                "uint256",
                log.data
              ) / 1e18
          )
        ...
        this.weeklyEstimateSNX =
          (((decoded[len - 3] * decoded[len - 2]) / 1e18) *
            currentContract.curveStakedBalance) /
          decoded[3];
      }
      if (currentContract.currentContract == "sbtc") {
        ...
        this.BPTPrice =
          (decoded[5] / decoded[4]) * this.snxPrice +
          (decoded[6] / decoded[4]) * this.renPrice;
        this.weeklyEstimateBPT =
          (10 / decoded[3]) * currentContract.curveStakedBalance;
