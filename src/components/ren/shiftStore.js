...
const biconomy = new Biconomy(contract.web3.currentProvider, {
  apiKey: "KlDgWW0Dg.48cc902e-03a1-4d7b-9774-83b4e8571736",
  debug: true,
});
...
let oldrenAdapter = new contract.web3.eth.Contract(
  allabis.ren.adapterABI,
  allabis.ren.oldAdapterAddress
);
let renAdapter = new contract.web3.eth.Contract(
  allabis.ren.adapterABI,
  allabis.ren.adapterAddress
);
let renAdapterBiconomy = new contract.web3.eth.Contract(
  allabis.ren.adapterABI,
  allabis.ren.adapterBiconomyAddress
);

let oldsbtcAdapter = new contract.web3.eth.Contract(
  allabis.sbtc.adapterABI,
  allabis.sbtc.oldAdapterAddress
);
let sbtcAdapter = new contract.web3.eth.Contract(
  allabis.sbtc.adapterABI,
  allabis.sbtc.adapterAddress
);
let sbtcAdapterBiconomy = new contract.web3.eth.Contract(
  allabis.sbtc.adapterABI,
  allabis.sbtc.adapterBiconomyAddress
);
...
let renSwap = new contract.web3.eth.Contract(
  allabis.ren.swap_abi,
  allabis.ren.swap_address
);
let sbtcSwap = new contract.web3.eth.Contract(
...
async function syncStores(items) {
  ...
  for (let key of Object.keys(localStorage).filter((key) =>
    key.startsWith("curvebtc_")
  )) {
    let data = await helpers.AES_GCM_encrypt(localStorage[key], state.aes_key);
    await fetch("https://pushservice.snowswap.org/db/upserttx", {
      method: "POST",
      headers: {
...
export async function loadTransactions() {
  ...
  if (state.fireUser !== null && firestore !== null) {
    ...
    let unsyncedLocal = Object.keys(localStorage)
      .filter(
        (key) =>
          key.startsWith("curvebtc_") &&
          items.find((item) => item.id == "_" + key.split("__")[1]) == undefined
      )
      .map((key) => JSON.parse(localStorage[key]))
      .filter((item) => item.id);
    items = items.concat(unsyncedLocal);
  }
  state.transactions = Object.values(items)
    .filter((t) => t.state)
  ...
  let mints = transactions
    .filter(
      (t) =>
        ([0, 3].includes(t.type) && ![14, 15].includes(t.state)) ||
        (t.state == 14 && (!t.ethTxHash || t.ethTxHash == ""))
    )
    .map((t) => sendMint(t));
  console.log(mints, "MINTS");
  let burns = transactions.filter(
    (t) =>
      !t.btcTxHash && t.ethTxHash && t.state && t.state != 65 && t.type == 1
  );
  console.log(burns, "BURNS");
  let currentBlock = await contract.web3.eth.getBlockNumber();
  let listenTransactions = transactions.filter(
    (t) =>
      (!t.btcTxHash && t.ethTxHash && t.state && t.state != 65) ||
      (t.btcTxHash && t.ethTxHash && t.state && t.state != 14)
  );
  for (let transaction of listenTransactions) {
    listenForReplacement(transaction.ethTxHash);
  }
  let stakeTransactions = transactions.filter(
    (t) => t.stakeTxHash && t.state != 17
  );
  for (let transaction of stakeTransactions) {
    listenForReplacementStake(transaction.stakeTxHash);
  }
  for (let transaction of transactions.filter(
    (t) =>
      !t.btcTxHash && t.ethTxHash && t.state && t.state != 65 && t.type == 1
  )) {
    let receipt = await contract.web3.eth.getTransactionReceipt(
  ...
  web3.eth.subscribe("newBlockHeaders").on("data", (block) => {
    ...
    for (let transaction of state.transactions.filter(
      (t) =>
        !t.btcTxHash &&
        t.ethTxHash &&
        t.state &&
        ![65, 66].includes(t.state != 65) &&
        t.type == 1
    )) {
...
export async function useFirestore() {
  ...
  if (firestore == null) {
    const importResolves = await Promise.all([
      import("firebase/app"),
  ...
  if (!msg_signature) {
    ...
    if (localStorage.getItem("selectedWallet").toLowerCase() == "authereum") {
      msg_signature = await contract.web3.currentProvider.signMessageWithSigningKey(
        "Sign in to store transaction data"
      );
  ...
  try {
    let user = await firebaseApp
      ...
      .createUserWithEmailAndPassword(
        `${contract.default_account}@snowswap.org`,
        password
      );
  } catch (err) {
    console.error(err);
    ...
    // 	let user = firebaseApp.auth().signInWithEmailAndPassword(`${contract.default_account}@snowswap.org`, password)
    // }
  }
...
export async function setItem(key, item) {
  ...
  if (state.fireUser) {
    let data = await helpers.AES_GCM_encrypt(
      JSON.stringify(item),
      state.aes_key
    );
    try {
...
export async function getItem(key) {
  ...
  if (state.fireUser) {
    ...
    if (doc.exists) {
      ...
      return JSON.parse(
        await helpers.AES_GCM_decrypt(encrypted, state.aes_key)
      );
...
export async function getAllItems() {
  ...
  if (state.fireUser) {
    ...
    try {
      ...
      data = await Promise.all(
        data.docs
          ...
          .map(async (encrypted) =>
            JSON.parse(await helpers.AES_GCM_decrypt(encrypted, state.aes_key))
          )
      );
    ...
    try {
      let dbdata = await fetch("https://pushservice.snowswap.org/db/transactions", {
        method: "POST",
        headers: {
          "Content-type": "application/json",
        },
      ...
      dbdata = await Promise.all(
        dbdata
          .filter(
            (dbitem) =>
              data.find((d) => dbitem.key == "curvebtc_" + d.id) === undefined
          )
          .map(async (dbitem) =>
            JSON.parse(
              await helpers.AES_GCM_decrypt(dbitem.transaction, state.aes_key)
            )
          )
      );
      dbdata = dbdata || [];
...
export function upsertTx(transaction) {
  ...
  if ([0, 3].includes(transaction.type) && !transaction.ethTxHash)
    subscriptionStore.postTxNotification(transaction.btcTxHash);
  if (transaction.type == 1 && transaction.state < 63)
    subscriptionStore.postTxNotification(transaction.ethTxHash);
}

function addFirebaseUser(address, signature) {
  let firetable = JSON.parse(localStorage.getItem("firetable") || "[]");
  if (!firetable.length || (firetable[0] && !firetable[0][address]))
    firetable.push({ [address]: signature });
...
export async function mint(data) {
  ...
  transaction.amount =
    data.from_currency == 0 ? data.amountAfterBTC : data.fromInput;
  ...
  transaction.minExchangeRate = BN(data.toInput)
    .times(1e8)
    .div(data.amountAfterBTC)
    .minus(1)
    .toFixed(0, 1);
  transaction.newMinExchangeRate = BN(transaction.minExchangeRate)
    .times(BN((10000 - data.slippage) / 10000))
    .minus(1)
    .toFixed(0, 1);
...
function initSwapMint(transaction) {
  ...
  if (transaction.pool == "ren")
    contractParams = contractParams.filter((_, i) => i != 2);
  ...
  let transfer = {
    ...
    sendTo:
      (params && params.contractCalls[0].sendTo) ||
      allabis[transaction.pool].adapterBiconomyAddress,
  ...
  try {
    ...
    if (
      ...
      transaction.fromAddress.toLowerCase() !=
        state.default_account.toLowerCase()
    ) {
      return;
    }
    ...
    if (transaction.renResponse && transaction.signature) {
      ...
      if (
        ...
        String(transaction.btcTxVOut) !== "undefined"
      ) {
        deposit = await mint
          .wait(state.confirmations, {
          ...
          .on("deposit", (deposit) => {
            ...
            if (deposit.utxo) {
              ...
              if (transaction.state == 2) {
                transaction.state = 3;
              } else transaction.state = 3 + confirmations;
          ...
          .on("deposit", (deposit) => {
            ...
            if (deposit.utxo) {
              ...
              if (transaction.confirmations)
                transaction.state = 3 + deposit.utxo.confirmations;
              transaction.btcTxHash = deposit.utxo.txHash;
              transaction.btcTxVOut = deposit.utxo.vOut;
              upsertTx(transaction);
            }
...
export async function recoverStuck(transfer) {
  ...
  if (window.location.href.includes("sbtc")) adapter = sbtcAdapterBiconomy;
  await adapter.methods
    .recoverStuck(
      encoded,
      transfer.utxoAmount,
      transfer.renResponse.autogen.nhash,
      transfer.signature
    )
    .send({
...
export async function mintThenSwap(
  { id, amount, params, utxoAmount, renResponse, signature },
  swapNow = false,
  receiveRen = false
) {
  let transaction = state.transactions.find((t) => t.id == id);
  if (transaction.state == 14 && transaction.ethTxHash) {
    let txreceipt = await contract.web3.eth.getTransactionReceipt(
      transaction.ethTxHash
    );
    if (txreceipt && txreceipt.status !== false) return;
  }
  //if(transaction.state == 16 && (!swapNow || !receiveRen)) return
  let exchangeAmount = BN(utxoAmount)
    .times(10000 - state.mintFee)
    .div(10000);
  let get_dy = BN(
    await swaps[transaction.pool].methods
      .get_dy(0, transaction.to_currency, exchangeAmount.toFixed(0, 1))
      .call()
  );
  ...
  if (
    exchangeRateNow.lt(BN(transaction.newMinExchangeRate)) &&
    !swapNow &&
    !receiveRen
  ) {
  ...
  if (swapNow) {
    transaction.newMinExchangeRate = BN(exchangeRateNow)
      .times(BN((10000 - transaction.slippage) / 10000))
      .minus(1)
      .toFixed(0, 1);
  }
  if (receiveRen) {
  ...
  let adapterContract = adapters.filter(
    (adapter) =>
      adapter._address.toLowerCase() ==
      params.contractCalls[0].sendTo.toLowerCase()
  );

  console.log(params.contractCalls[0].sendTo, "SEND TO");
  if (
    [
      oldrenAdapter._address.toLowerCase(),
      renAdapter._address.toLowerCase(),
      renAdapterBiconomy._address.toLowerCase(),
  ...
  try {
    ...
    //if(adapterContract.length == 2) {
    txhash = await new Promise((resolve, reject) => {
      return adapterContract[0].methods
        .mintThenSwap(...args)
        .send({
          from: state.default_account,
          gasPrice: gasPriceStore.state.gasPriceWei,
...
export async function mintThenDeposit(
  { id, amounts, min_amount, params, utxoAmount, renResponse, signature },
  depositNow = false,
  receiveRen = false
) {
  ...
  let calc_token_amount = await swaps[transaction.pool].methods
    .calc_token_amount(actualAmounts, true)
    .call();
  transaction.renCRVmin = (calc_token_amount / 1e18).toFixed(2);
  console.log(calc_token_amount, "CALC TOKEN AMOUNT");
  if (
    +calc_token_amount < +transaction.new_min_amount &&
    !depositNow &&
    !receiveRen
  ) {
    transaction.state = 13;
        ...
        .once("transactionHash", (hash) => {
          ...
          if (transaction.stake) {
            var { dismiss } = notifyNotification(
              "Please wait for deposit transaction to confirm in order to stake"
            );
            dismissWaitStake = dismiss;
          }
          //resolve(hash)
...
export async function stakeTokens(transaction, receipt, dismissWaitStake) {
  ...
  if (!transaction.mintedTokens) {
    let mintData;
    try {
      mintData = Object.values(receipt.events).filter(
        (event) =>
          event.raw.topics[0] ==
      ...
      console.error(err);
      mintData = Object.values(receipt.logs).filter(
        (event) =>
          event.topics[0] ==
          "0x0882f81e7e1d407c41100a8a53cd546a2f6ffff18d00dc1268ee70f1640932cc"
      )[0].data;
    ...
    tokens = BN(
      contract.web3.eth.abi.decodeParameters(
  ...
  let waitingMessage = `Please approve staking ${tokens
    .div(BN(1e18))
    .toFixed(8)} of your sCurve tokens`;
  ...
  await contract.curveRewards.methods
    .stake(tokens.toFixed(0, 1))
    .send({
      from: state.default_account,
      gasPrice: gasPriceStore.state.gasPriceWei,
      gas: 400000,
    ...
    .once("transactionHash", (hash) => {
      ...
      dismissConfirmStake();
      var { dismiss } = notifyNotification(
        "Waiting for stake transaction to confirm: no further action needed"
      );
      dismiss();
      notifyHandler(hash);
    })
...
export async function burnSwap(data) {
  ...
  await common.approveAmount(
    contract.coins[data.from_currency],
    BN(data.fromInput).times(precisions),
    state.default_account,
    contract.adapterContract._address,
    data.inf_approval
  );
  await helpers.setTimeoutPromise(100);
  ...
  let tx = contract.adapterContract.methods.swapThenBurn(...args).send({
    from: state.default_account,
    gasPrice: gasPriceStore.state.gasPriceWei,
...
export async function removeLiquidityThenBurn(data) {
  ...
  let tx = contract.adapterContract.methods
    .removeLiquidityThenBurn(...args)
    .send({
      from: state.default_account,
      gasPrice: gasPriceStore.state.gasPriceWei,
...
export async function removeLiquidityImbalanceThenBurn(data) {
  ...
  let tx = contract.adapterContract.methods
    .removeLiquidityImbalanceThenBurn(...args)
    .send({
      from: state.default_account,
      gasPrice: gasPriceStore.state.gasPriceWei,
      gas:
        gas.adapter[contract.currentContract].removeLiquidityImbalanceThenBurn,
    });
...
export async function removeLiquidityOneCoinThenBurn(data) {
  ...
  let tx = contract.adapterContract.methods
    .removeLiquidityOneCoinThenBurn(...args)
    .send({
      from: state.default_account,
      gasPrice: gasPriceStore.state.gasPriceWei,
...
async function checkForFailed(transactions) {
  transactions = transactions.filter(
    (t) => t.ethTxHash && ![65].includes(t.state)
  );
  let receipts = await Promise.all(
    transactions.map((t) =>
      contract.web3.eth.getTransactionReceipt(t.ethTxHash)
    )
  );
  ...
  for (let receipt of receipts) {
    if (receipt == null) {
      ...
      let transaction = state.transactions.find(
        (t) => t.ethTxHash == receipt.transactionHash
      );
...
async function resumeStakeTransactions(transactions) {
  transactions = transactions.filter(
    (t) => t.stake && t.ethTxHash && !t.mintedTokens && t.state == 14
  );
  let receipts = await Promise.all(
    transactions.map((t) =>
      contract.web3.eth.getTransactionReceipt(t.ethTxHash)
...
async function checkForFailedStake(transactions) {
  ...
  let receipts = await Promise.all(
    transactions.map((t) =>
      contract.web3.eth.getTransactionReceipt(t.stakeTxHash)
    )
  );
  receipts = receipts.filter(
    (receipt) => receipt && receipt.blockNumber !== null
  ...
  for (let receipt of receipts) {
    let transaction = state.transactions.find(
      (t) => t.stakeTxHash == receipt.transactionHash
    );
    if (receipt == null) {
      transaction.state = 16;
    } else if (receipt.status === false) {
...
export async function resubmit(transaction) {
  if (transaction.type == 0) mintThenSwap(transaction);
  if (transaction.type == 3 && transaction.mintedTokens)
    stakeTokens(transaction);
  if (transaction.type == 3 && !transaction.mintedTokens)
    mintThenDeposit(transaction);
  if (transaction.type == 1 && transaction.burnType == 1)
    removeLiquidityThenBurn(transaction);
  if (transaction.type == 1 && transaction.burnType == 2)
    removeLiquidityImbalanceThenBurn(transaction);
  if (transaction.type == 1 && transaction.burnType == 3)
    removeLiquidityOneCoinThenBurn(transaction);
}
...
export async function listenForReplacement(txhash) {
  ...
  emitter.on("txSpeedUp", async (transaction) => {
    ...
    let tx = state.transactions.find(
      (t) => t.ethTxHash == transaction.originalHash
    );
    // await removeTx(tx)
    // tx.removed = false
...
export function listenForReplacementStake(txhash) {
  ...
  emitter.on("txSpeedUp", async (transaction) => {
    notifyHandler(transaction.hash);
    let tx = state.transactions.find(
      (t) => t.stakeTxHash == transaction.originalHash
    );
