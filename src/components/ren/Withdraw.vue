<template>
  <div class="add-liquidity">
    <fieldset class="percentage">
      <legend>
        ...
        <input
          ...
          v-model="showstaked"
        />
        <label for="showstaked" v-show="currentPool == 'sbtc'">
          Show staked
        </label>
      </legend>
      <ul>
        ...
        <li v-for="(currency, i) in Object.keys(currencies)">
          <label :for="'currency_' + i" class="currency_label">
            ...
            <span v-show="withdrawc">
              ...
              <span
                v-show="
                  !(currency == 'usdt' && currentPool == 'usdt') &&
                  !['susdv2', 'ren', 'sbtc'].includes(currentPool)
                "
              >
            ...
            <span
              v-show="
                !withdrawc && !['susdv2', 'tbtc', 'ren'].includes(currentPool)
              "
            >
              {{ currency | capitalize }}
            </span>
      ...
      <ul>
        ...
        <li>
          ...
          >
            ...
            <span class="tooltip">
              [?]
              <span class="tooltiptext">(may use less gas)</span>
            </span>
          </label>
        </li>
      </ul>
    ...
    <div id="withdraw_buttons" class="buttons">
      ...
      <p
        class="simple-error invalid-address"
        v-show="btcAddress && !checkAddress"
      >
      ...
      <button
        ...
        :disabled="
          slippage < -0.03 || !btcAddress || amountAfterBTC < 0 || !checkAddress
        "
      ...
      <button
        ...
        @click="claim_SNX"
        v-show="
          ['susdv2', 'sbtc'].includes(currentPool) && pendingSNXRewards > 0
        "
      >
...
export default {
  ...
  computed: {
    ...
    amountAfterBTC() {
      return BN(this.inputs[0])
        .times(1e8)
        .times(1 - state.mintFee / 10000)
        .minus(state.minersLockFee)
        .div(1e8)
        .toFixed(8);
    },
    minOrderSize() {
      return (
        (state.minersReleaseFee + state.burnFee / 10000) / 1e8 +
        0.00000547
      ).toFixed(8);
    },
    gasPrice() {
  ...
  methods: {
    ...
    toFixed(num, precisions = 2, round = 4) {
      ...
      if (
        precisions == 2 &&
        ["tbtc", "ren", "sbtc"].includes(currentContract.currentContract)
      )
        precisions = 8;
    ...
    async handle_change_amounts(i, event) {
      ...
      try {
        ...
        if (
          (i == 1 &&
            availableAmount.gt(
              maxAvailableAmount.plus(BN(this.staked_balance))
            )) ||
          (i == 0 && this.amountAfterBTC < 0)
        ) {
    ...
    async claim_SNX() {
      ...
      this.waitingMessage = `Claiming ${(this.pendingSNXRewards / 1e18).toFixed(
        2
      )} SNX`;
    ...
    async unstake(amount, exit = false, unstake_only = false) {
      if (unstake_only)
        ...
        this.waitingMessage = `
                    Need to unstake ${this.toFixed(
                      amount.div(BN(1e18))
                    )} tokens from Mintr for withdrawal.
                    <br>
    ...
    async handle_remove_liquidity(unstake = false, unstake_only = false) {
      ...
      if (this.share == "---") {
        ...
        if (
          (this.token_balance.lt(BN(token_amount)) || unstake) &&
          ["susdv2", "sbtc"].includes(this.currentPool)
        )
          await this.unstake(
            BN(token_amount).minus(BN(this.token_balance)),
        ...
        try {
          await common.ensure_allowance_zap_out(
            token_amount,
            undefined,
            allabis[currentContract.currentContract].adapterBiconomyAddress
          );
          var { dismiss } = notifyNotification(
            "Please confirm withdrawal transaction"
          );
        ...
        if (
          (this.token_balance.lt(BN(amount)) || unstake) &&
          ["susdv2", "sbtc"].includes(this.currentPool)
        )
          await this.unstake(
            BN(amount).minus(BN(this.token_balance)),
            unstake && !unstake_only,
            unstake_only
          );
        if (unstake_only) return;
        ...
        if (this.to_currency !== null && this.to_currency < 10) {
          ...
          let min_amount;
          try {
            min_amount = await currentContract.swap.methods
          ...
          await store.removeLiquidityOneCoinThenBurn({
            address: this.btcAddress,
            coinDestination: currentContract.default_account,
            token_amounts: amount,
            renBTCAmount: this.inputs[0],
            min_amount: BN(min_amount)
          ...
          try {
            ...
            await store.removeLiquidityThenBurn({
              address: this.btcAddress,
              coinDestination: currentContract.default_account,
              amount: amount,
              renBTCAmount: this.inputs[0],
