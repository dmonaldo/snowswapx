<template>
  <div class="window half-width info">
    <div id="lp-info-currency">
      <div class="gray-block">
        <h3>Currency reserves</h3>

        <ul id="balances-info">
          <li v-for="(currency, i) in Object.keys(currencies)">
            <b>{{ currencies[currency] | capitalize }}:</b>

            <!--            <span-->
            <!--              :class="{ 'loading line': !bal_info || bal_info[i] === null }"-->
            <!--            >-->
            <span v-show="bal_info && bal_info[i]">
              {{ bal_info && toFixed(bal_info[i]) }} ({{
                ((bal_info && bal_info[i] * 100) / totalBalances) | toFixed2
              }}%)
              <!--              </span>-->
            </span>
          </li>

          <li class="balances-info__total-currencies">
            <b>{{ totalCurrencies(currencies) }}:</b>

            <!--            <span :class="{ 'loading line': totalBalances === null }">-->
            <span>
              {{ toFixed(totalBalances) }}
            </span>
          </li>
          ...
          <li v-show="['ren', 'sbtc', 'hbtc'].includes(currentPool)">
            ...
            <!--            <span :class="{ 'loading line': totalBalances === null }">-->
            <span>{{ (totalBalances * btcPrice) | formatNumber(2) }}$</span>
          </li>
        </ul>
      </div>

      <div
        class="gray-block lp-info-currency__share"
        v-show="totalShare > 0 && initializedContracts"
      >
        <h3>
          My share:
          <br />
          <span>
            ({{ ((totalBalance / totalSupply) * 100).toFixed(3) }}% of pool)
          </span>
        </h3>
        ...
        <ul id="lp-info">
          <li v-for="(currency, i) in Object.keys(currencies)">
            ...
            <span>{{ l_info && toFixed(l_info[i]) }}</span>
          </li>

          <li>
            <b>{{ totalCurrencies(currencies) }}:</b>

            <!--            <span :class="{ 'loading line': totalUserBalances === null }">-->
            <span>
              {{ toFixed(totalUserBalances) }}
            </span>
          </li>
        </ul>

        <!--        <ul>-->
        <!--          <li v-show="realShare !== null">-->
        <!--            <b>USD balance:</b>-->

        <!--            <span>-->
        <!--              &lt;!&ndash;              <span :class="{ 'loading line': realShare === null }">&ndash;&gt;-->
        ...
        <!--          <li>-->
        <!--            <b>Averaged USD balance:</b>-->

        <!--            <span>{{ usdShare1 | formatNumber(2) }}</span>-->
        <!--          </li>-->
        <!--        </ul>-->
      </div>

      <div class="gray-block lp-info-currency__other">
        <ul>
          <li>
            <b>Fee:</b>
            <!--            <span id="fee-info" :class="{ 'loading line': !fee }"></span>-->
            <span v-show="fee">{{ fee && fee.toFixed(3) }}%</span>
          </li>

          <li>
            <b>Admin fee:</b>
            <!--            <span id="admin-fee-info" :class="{ 'loading line': admin_fee }">-->
            <span>{{ admin_fee && admin_fee.toFixed(3) }}%</span>
          </li>
        </ul>
        ...
        <ul>
          ...
          <li>
            ...
            <b>
              ...
              <!--                Amplification coefficient chosen from fluctation of prices-->
              <!--                around 1-->
              <!--              </span>-->
            </b>
            <!--            <span :class="{ 'loading line': A1 === null }">-->
            <span>
              {{ A1 }}
            </span>
          </li>
        ...
        <!--                [?]-->
        <!--                <span class="tooltiptext long">-->
        <!--                  Slowly changing up A so that it doesn't negatively-->
        <!--                  changevirtual price growth of shares-->
        <!--                </span>-->
        <!--              </span>-->
        <!--            </b>-->
        ...
        <!--          <li-->
        <!--            v-show="-->
        <!--              admin_actions_deadline1 !== null && admin_actions_deadline1 !== 0-->
        <!--            "-->
        <!--          >-->
        <!--            <b>Future A:</b>-->
        <!--            <span :class="{ 'loading line': future_A1 === null }">-->
        <!--              {{ future_A1 }}-->
        <!--            </span>-->
        <!--          </li>-->
        <!--          <li-->
        ...
        <ul
          v-show="
            (totalBalances !== null && nativeVolume != -1) || poolVolumeUSD > -1
          "
        >
          <li v-show="totalBalances !== null && nativeVolume != -1">
            <b>Liquidity utilization:</b>

            <span
              :class="{
                'loading line': totalBalances === null || nativeVolume == -1,
              }"
            >
            ...
            <span class="tooltip">
              [?]
              <span class="tooltiptext">24h Volume/Liquidity ratio</span>
            </span>
          </li>

          <li v-show="poolVolumeUSD > -1">
            <b>Daily USD volume:</b>
            <span :class="{ 'loading line': poolVolumeUSD == -1 }">
              <span v-show="poolVolumeUSD > -1">
                ${{ poolVolumeUSD && poolVolumeUSD | formatNumber(2) }}
              </span>
            </span>
          </li>
      ...
      <!--      <div v-show="totalStake > 0 && initializedContracts">-->
      <!--        <h3>Staked share:</h3>-->

      <!--        <span>-->
      <!--          ( {{ ((totalStake / totalSupply) * 100).toFixed(3) }}% of pool)-->
      <!--        </span>-->
      <!--        <ul id="stakelp-info">-->
      <!--          <li v-for="(currency, i) in Object.keys(currencies)">-->
      <!--            <b>{{ currency | capitalize }}:</b>-->
      <!--            <span>{{ staked_info && toFixed(staked_info[i]) }}</span>-->
      <!--          </li>-->
      <!--          <li>-->
      <!--            <b>{{ totalCurrencies(currencies) }}:</b>-->
      <!--            <span :class="{ 'loading line': totalStakedBalances === null }">-->
      <!--              {{ toFixed(totalStakedBalances) }}-->
      <!--            </span>-->
      <!--          </li>-->
      <!--          <li>-->
      <!--            <b>USD balance:</b>-->
...
export default {
  ...
  methods: {
    toFixed(num, precisions = 2, round = 4) {
      ...
      if (
        precisions == 2 &&
        ["tbtc", "ren", "sbtc", "hbtc"].includes(this.currentPool)
      )
        precisions = 8;
          ...
          let dy = await calcWorker.calcPrice(
            {
              ...
              PRECISION_MUL: allabis[this.currentPool].coin_precisions.map(
                (p) => 1e18 / p
              ),
              PRECISION: 1e18,
            },
            i,
            1,
            amount * allabis[this.currentPool].coin_precisions[i]
          );
  ...
  computed: {
    ...
    totalBalances() {
      return (
        (this.bal_info && this.bal_info.reduce((a, b) => a + b, 0)) || null
      );
    },
    totalUserBalances() {
    ...
    totalStakedBalances() {
      return (
        (this.staked_info && this.staked_info.reduce((a, b) => a + b, 0)) ||
        null
      );
    },
    usdShare1() {
    ...
    isRampingUp() {
      return (
        getters.initial_A_time() < Date.now() / 1000 < getters.future_A_time()
      );
    },
    rampEnd() {
    ...
    hasLoadedInfo() {
      ...
      return (
        (this.currentPool &&
          this.l_info &&
          this.l_info[N_COINS - 1] !== undefined) ||
        (this.staked_info && this.staked_info[N_COINS - 1] !== undefined)
      );
    },
  },
};
