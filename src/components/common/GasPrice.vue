<template>
  <div>
    ...
    <div id="gas_price" v-show="gasPriceMedium">
      <h4 class="gas_price__heading">Gas price</h4>

      <div class="gas_price__content">
        ...
        <input
          ...
          type="radio"
          name="gas"
          :value="gasPriceMedium"
          @click="
            customGasDisabled = true;
            gasPrice = gasPriceMedium;
          "
        />
        <label for="gasstandard">
          <span class="gas_price__label">Standard</span>
          <span class="gas_price__value">{{ Math.round(gasPriceMedium) }}</span>
        </label>
        ...
        />
        <label for="gasfast">
          <span class="gas_price__label">Fast</span>
          <span class="gas_price__value">{{ Math.round(gasPriceFast) }}</span>
        </label>
        ...
        <input
          ...
          type="radio"
          name="gas"
          :value="gasPriceFastest"
          @click="
            customGasDisabled = true;
            gasPrice = gasPriceFastest;
          "
        />
        <label for="gasinstant">
          <span class="gas_price__label">Instant</span>
          <span class="gas_price__value">
            {{ Math.round(gasPriceFastest) }}
          </span>
        </label>
        <input
          id="custom_gas"
          ...
          value="-"
          @click="
            customGasDisabled = false;
            gasPrice = gasPriceSlow;
          "
        />
        <label
          for="custom_gas"
        ...
        >
          <span
            class="gas_price__label"
            v-show="customGasPriceInput == gasPriceSlow"
          >
            Slow
          </span>
          <span class="gas_price__value">{{ Math.round(gasPriceSlow) }}</span>
  ...
  methods: {
    async getGasPrice() {
      try {
        ...
        try {
          let gasPriceInfo = await retry(
            fetch("https://fees.upvest.co/estimate_eth_fees")
          );
          gasPriceInfo = await gasPriceInfo.json();
