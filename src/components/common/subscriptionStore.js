...
export async function subscribeNotifications() {
  let swregistration = await navigator.serviceWorker.ready;
  let response = await fetch("https://pushservice.snowswap.org/vapidPublicKey");
...
export async function unsubscribeNotifications() {
  ...
  await fetch("https://pushservice.snowswap.org/removeSubscription", {
    method: "POST",
    headers: {
...
export async function updateSubscriptionServer(newSubscription) {
  if (state.subscription === null && !newSubscription)
    registerSubscription(newSubscription);
  await fetch("https://pushservice.snowswap.org/updateSubscription", {
    method: "POST",
    headers: {
      "Content-type": "application/json",
    },
...
export async function registerSubscription(subscription) {
  await fetch("https://pushservice.snowswap.org/register", {
    method: "post",
    headers: {
      "Content-type": "application/json",
    },
...
export async function postTxNotification(txHash) {
  ...
  return fetch("https://pushservice.snowswap.org/addtx", {
    method: "POST",
    headers: {
      "Content-type": "application/json",
...
export async function removeTxNotification(txHash) {
  ...
  console.log(subscription);
  if (subscription === null) return;
  return fetch("https://pushservice.snowswap.org/removetx", {
    method: "POST",
    headers: {
      "Content-type": "application/json",
    },
