<template>
  <div>
    <p
      ...
      v-show="showSlippage && slippage < -0.005"
    >
      <span class="text">
        ...
        <span class="tooltip">
          ...
          <span class="tooltiptext long">
            Slippage comes from depositing too many coins not in balance, and
            current coin prices are additionally accounted for
          </span>
        </span>
      </span>
    </p>
    ...
    <!--      class="info-message"-->
    <!--      id="slippage-warning"-->
    <!--      v-show="showSlippage && slippage <= 0 && slippage >= -0.005"-->
    <!--    >-->
    <!--      <span class="text"> Slippage(plus pricing): </span>-->
    <!--      <span class="percent">{{ (-slippage * 100).toFixed(3) }}</span-->
    <!--      >%-->
    <!--      <span class="tooltip"-->
    <!--        >[?]-->
    ...
    <p class="success" id="bonus-window" v-show="showSlippage && slippage > 0">
      Bonus(plus pricing):
      <span>{{ (slippage * 100).toFixed(3) }}</span>
      %
      <span class="tooltip">
        [?]
        <span class="tooltiptext long">
          Bonus comes as an advantage from current coin prices which usually
          appears for coins which are
          {{ $route.name == "Deposit" ? "low" : "high" }} in balance
        </span>
      </span>
    </p>
    ...
    <p
      ...
      id="susd-warning"
      v-show="currentPool == 'susdv2' && showSlippage && slippage < -0.01"
    >
...
export default {
  ...
  computed: {
    ...
    noBalanceText() {
      ...
      if (
        !(this.show_nobalance_i == 2 && this.currentPool == "usdt") &&
