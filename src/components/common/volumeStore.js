...
export const state = Vue.observable({
  volumes: {
    ...Object.fromEntries(
      Object.entries(initState()).map(([key, el]) => [key, [-1, -1]])
    ),
  },
  volumeData: {
    5: {
...
export async function fetchVolumeData(pools, refresh = false, period = 5) {
  ...
  if (pools.includes("tbtc") || pools.includes("ren") || pools.includes("hbtc"))
      ...
      data = data.map((d) => {
        d.volume = Object.fromEntries(
          Object.entries(d.volume).map(([k, v]) => [
            k,
            v.map(
              (vol) => vol * findClosestPrice(d.timestamp, btcPrices.prices)
            ),
          ])
        );
        return d;
...
export async function getVolumes(pools, refresh = false) {
  ...
  if (
    Object.values(state.volumes).filter((v) => v[0] != -1).length ==
      pools.length &&
    !refresh
  )
    return;
...
export async function getDailyVolume(pool, refresh = false, interval = 30) {
  ...
  for (let data of json) {
    state.allVolume[pool].push([
      data.timestamp * 1000,
      Object.entries(data.volume)
        .map(([k, v]) => {
          let precisions =
            abis[pool == "susd" ? "susdv2" : pool].coin_precisions[
              k.split("-")[0]
            ];
...
export async function getLendingAPY(pool, refresh = false, interval = 30) {
  ...
  for (let j = 1; j < state.volumeData[interval][pool].length; j++) {
    ...
    let balances = data.balances.map(
      (b, bi) =>
        (b /= abis[pool == "susd" ? "susdv2" : pool].coin_precisions[bi])
    );
