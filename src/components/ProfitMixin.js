...
export default {
  ...
  computed: {
    ...
    stakeAddresses() {
      ...
      rewardsContracts.push(
        ...Object.values(allabis)
          .filter((pool) => pool.gauge_address)
          .map(
            (pool) => "0x000000000000000000000000" + pool.gauge_address.slice(2)
          )
      );
    ...
    showEarnedSNXGauge() {
      if (this.showinUSD == 1)
        return ((+this.claimableSNX / 1e18) * this.SNXprice).toFixed(2);
      return (+this.claimableSNX / 1e18).toFixed(2);
    },
    showRewardsSNXGauge() {
      if (this.showinUSD == 1)
        return ((+this.claimedSNX / 1e18) * this.SNXprice).toFixed(2);
  ...
  methods: {
    async mounted() {
      try {
        this.account =
          this.$route.params.address || currentContract.default_account;
        ...
        for (let [i, symbol] of Object.values(
          allCurrencies[this.currentPool]
        ).entries()) {
        ...
        let calls = [
          [
            currentContract.minter._address,
            currentContract.minter.methods
              .minted(
          ...
          [
            ...
            currentContract.gaugeContract.methods
              .claimable_tokens(currentContract.default_account)
              .encodeABI(),
          ],
        ];
          ...
          calls.push(
            ...[
              [
                currentContract.gaugeContract._address,
                currentContract.gaugeContract.methods
                  .claimable_reward(currentContract.default_account)
                  .encodeABI(),
              ],
              [
                currentContract.gaugeContract._address,
                currentContract.gaugeContract.methods
                  .claimed_rewards_for(currentContract.default_account)
                  .encodeABI(),
              ],
            ]
          );
        ...
        let CRVprice = await fetch(
          `https://api.coingecko.com/api/v3/simple/price?ids=havven,curve-dao-token&vs_currencies=usd`
        );
        ...
        this.profit =
          available + this.getStakedBalance + this.withdrawals - this.deposits;
        console.log(
          available,
    ...
    async getAvailableAmount() {
      if (!this.$route.params.address) {
        ...
        if (["y", "iearn"].includes(this.currentPool)) {
          let vault = "0x5dbcF33D8c2E976c6b560249878e6F1491Bca25c";  // do we need to change
          let vaultContract = new currentContract.web3.eth.Contract(
            yERC20_abi,
          ...
          let calls = [
            ...
            [
              vaultContract._address,
              vaultContract.methods.getPricePerFullShare().encodeABI(),
            ],
          ];
          let aggcalls = await currentContract.multicall.methods
            .aggregate(calls)
            .call();
          let [balanceOf, rate] = aggcalls[1].map(
            (hex) =>
              +currentContract.web3.eth.abi.decodeParameter("uint256", hex)
          );

          if (balanceOf > 0) this.isInVault = true;
          ...
          usdShare +=
            ((+balanceOf * +rate) / 1e36) * currentContract.virtual_price;
        }
        if (["susdv2", "sbtc", "y"].includes(this.currentPool)) {
          let oldStakedRewards = await currentContract.curveRewards.methods
            .balanceOf(currentContract.default_account)
            .call();
        ...
        return [
          totalShare * 100,
          usdShare * this.btcPrice || 0,
          (stakedBalance * currentContract.virtual_price * this.btcPrice) /
            1e18,
          currentContract.totalStake * 100,
        ];
      }
    ...
    async calcAvailable() {
      let calls = [
        [
          this.CURVE_TOKEN,
          "0x70a08231000000000000000000000000" + this.account.slice(2),
        ],
      ];
      if (
        ["susdv2", "sbtc", "y", "iearn"].includes(
      ...
      let aggcalls = await currentContract.multicall.methods
        .aggregate(calls)
        .call();
      let [tokenBalance, stakedBalance] = aggcalls[1].map(
        (hex) => +currentContract.web3.eth.abi.decodeParameter("uint256", hex)
      );
      let totalStake = 0;
      if (["susdv2", "sbtc"].includes(this.currentPool)) {
        ...
        totalStake = currentContract.bal_info
          .map((bal, i) => {
      ...
      let totalShare = currentContract.bal_info
        .map((bal, i) => {
    ...
    async calculateAvailable(prices) {
      ...
      for (let i = 0; i < prices.length; i++) {
        ...
        if (["DAI", "USDC", "USDT", "sUSD"].includes(curr)) {
          ...
          available += this.fromNativeCurrent(
            curr,

            BN(exchangeRate).times(BN(prices[i])).div(BN(1e8))
          );
        }
      }
      return available;
    ...
    async convertValuesCurrent(curr) {
      ...
      return (value) => {
        return this.fromNativeCurrent(
          curr,
          BN(usdPool).times(BN(value)).div(BN(tokensSupply)).times(BN(100))
        );
      };
    },
    ...
    async getClosestBTCPrice(timestamp) {
      while (true) {
        try {
          let req = await fetch(
            `https://api.coingecko.com/api/v3/coins/bitcoin/market_chart/range?vs_currency=usd&from=${timestamp}&to=${
              (Date.now() / 1000) | 0
            }`
          );
    ...
    async interpolatePoint(timestamp, priceData) {
      ...
      let prev = priceData.find(
        (p) => timestamp - p.timestamp > 0 && p.virtual_price > 0
      );
      let next = priceData.find(
        (p) => p.timestamp - timestamp > 0 && p.virtual_price > 0
      );
      ...
      if (Object.keys(point).length === 0 && point.constructor === Object) {
        ...
        point.virtual_price = interpolator(
          prev.virtual_price,
          next.virtual_price
        );
        point.balances = prev.balances.map((b, i) =>
          interpolator(b, next.balances[i])
        );
    ...
    async checkExchangeRateBlocks(block, address, direction) {
      ...
      if (mints.length) {
        let mint = mints[0];
        let mintevent = currentContract.web3.eth.abi.decodeParameters(
    ...
    async getExchangeRate(blockNumber, address, value, type) {
      if (
        currentContract.currentContract == "pax" &&
        address.toLowerCase() == currentContract.coins[3]._address.toLowerCase()
      ) {
        return 1;
      }
      let exchangeRate = await this.checkExchangeRateBlocks(
        blockNumber,
        address,
        0
      );
      ...
      if (exchangeRate === false) {
        ...
        let j = blockNumber;
        while (
          (exchangeRatePast = await this.checkExchangeRateBlocks(
            i,
            address,
            -1,
            type
        ...
        while (
          (exchangeRateFuture = await this.checkExchangeRateBlocks(
            j,
            address,
            1,
            type
        ...
        if (
          exchangeRatePast.exchangeRate === undefined ||
          exchangeRateFuture.exchangeRate === undefined
        )
          return -1;
        exchangeRate = interpolate(
          blockNumber,
          exchangeRatePast.blockNumber,
          exchangeRateFuture.blockNumber
    ...
    async calculateAmount(cTokens, block, type) {
      ...
      for (let i = 0; i < currentContract.N_COINS; i++) {
        ...
        if (["iearn", "busd", "pax"].includes(currentContract.currentContract))
          address = currentContract.underlying_coins[i]._address;
        ...
        if (
          ["iearn", "busd", "pax"].includes(currentContract.currentContract)
        ) {
          if (i == 0 || i == 3) tokens /= 1e16;
          else tokens /= 1e4;
          usd = tokens * exchangeRate;
        } else if (
          currentContract.currentContract == "susdv2" ||
          (currentContract.currentContract == "pax" && i == 3)
        ) {
          usd = this.fromNativeCurrent(
            curr,
            BN(exchangeRate).times(BN(tokens))
          );
        } else {
    ...
    async getDeposits() {
      ...
      if (
        localStorage.getItem(this.currentPool + "dversion") == this.version &&
        localStorage.getItem(this.currentPool + "lastDepositBlock") &&
        localStorage.getItem(this.currentPool + "dlastAddress") ==
          default_account &&
        currentContract.default_account
      ) {
        let block = +localStorage.getItem(
          this.currentPool + "lastDepositBlock"
        );
        fromBlock = "0x" + parseInt(block + 1).toString(16);
        depositUsdSum += +localStorage.getItem(
          this.currentPool + "lastDeposits"
        );
        this.depositsUSD = allDepositsUSD = +localStorage.getItem(
          this.currentPool + "lastDepositsUSD"
        );
      ...
      var lastBlock =
        (poolTokensReceivings.length &&
          poolTokensReceivings[poolTokensReceivings.length - 1].blockNumber) ||
        fromBlock;
      ...
      for (const hash of txs) {
        if (this.cancel) throw new Error("cancel");
        const receipt = await currentContract.web3.eth.getTransactionReceipt(
          hash
        );
        //old susd contract
        let timestamp = (
          await currentContract.web3.eth.getBlock(receipt.blockNumber)
        ).timestamp;
        console.log(timestamp);
        let addliquidity = receipt.logs.filter(
          (log) => log.topics[0] == this.addliquidityTopic
        );
        let removeliquidity = receipt.logs.filter(
          (log) => log.topics[0] == this.removeliquidityTopic
        );
        let removeliquidityImbalance = receipt.logs.filter(
          (log) => log.topics[0] == this.removeliquidityImbalanceTopic
        );
        console.log(addliquidity);
        let poolInfoPoint = await this.interpolatePoint(timestamp);
        let transfer = receipt.logs.filter(
          (log) =>
            log.address == this.CURVE_TOKEN &&
        ...
        console.log(
          transferTokens / 1e18,
          poolInfoPoint.virtual_price,
          (transferTokens * poolInfoPoint.virtual_price) / 1e36
        );
        if (
          addliquidity.length == 0 &&
          this.stakeAddresses.includes(transfer[0].topics[1])
        )
          continue;
        ...
        } else this.depositsUSD += depositsUSD;
        console.log(transferTokens);
        if (addliquidity.length) {
          let cTokens = currentContract.web3.eth.abi.decodeParameters(
            this.decodeParameters,
            addliquidity[0].data
          )[0];
          console.log(cTokens, "add liquidity cTokens");
          let usd = await this.calculateAmount(
            cTokens,
            receipt.blockNumber,
            "deposit"
          );
    ...
    async getWithdrawals(address) {
      ...
      let fromBlock = this.fromBlock;
      if (
        localStorage.getItem(this.currentPool + "wversion") == this.version &&
        localStorage.getItem(this.currentPool + "lastWithdrawalBlock") &&
        localStorage.getItem(this.currentPool + "wlastAddress") ==
          default_account &&
        currentContract.default_account
      ) {
        let block = +localStorage.getItem(
          this.currentPool + "lastWithdrawalBlock"
        );
        fromBlock = "0x" + parseInt(block + 1).toString(16);
        withdrawals += +localStorage.getItem(
          this.currentPool + "lastWithdrawals"
        );
        this.withdrawalsUSD = allWithdrawalsUSD = +localStorage.getItem(
          this.currentPool + "lastWithdrawalsUSD"
        );
        if (["ren", "sbtc", "hbtc"].includes(this.currentPool)) {
      ...
      var lastBlock =
        (logs.length && logs[logs.length - 1].blockNumber) || fromBlock;
      ...
      for (let log of logs) {
        ...
        if (this.removeliquidityOneTopic)
          removeliquidityOne = receipt.logs.filter(
            (log) => log.topics[0] == this.removeliquidityOneTopic
          );
        let transfer = receipt.logs.filter(
          (log) =>
            log.topics[0] == this.TRANSFER_TOPIC &&
            ...
            log.address.toLowerCase() ==
              currentContract.token_address.toLowerCase()
        );
        if (!transfer.length) continue;
        ...
        } else this.withdrawalsUSD += withdrawalsUSD;
        if (removeliquidity.length) {
          let cTokens = currentContract.web3.eth.abi.decodeParameters(
            this.decodeParametersWithdrawal,
            removeliquidity[0].data
          )[0];
        ...
        if (removeliquidityImbalance.length) {
          ...
          let decoded = currentContract.web3.eth.abi.decodeParameters(
            decodeParameters,
            removeliquidityImbalance[0].data
          );
        ...
        if (removeliquidityOne.length) {
          let decoded = currentContract.web3.eth.abi.decodeParameters(
            this.decodeParametersWithdrawalOne,
            removeliquidityOne[0].data
          );
    ...
    getAvailableTransfer(amount, poolInfoPoint) {
      return poolInfoPoint.balances
        .map((balance, i) => {
          return (
            (((balance * poolInfoPoint.rates[i]) /
              allabis[this.currentPool].coin_precisions[i]) *
              amount) /
            poolInfoPoint.supply /
            1e18
          );
        })
        .reduce((a, b) => a + b, 0);
