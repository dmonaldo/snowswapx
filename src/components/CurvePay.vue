    ...
    <fieldset>
      ...
      <div class="paycontainer">
        ...
        <select class="tvision" v-model="token">
          <option v-for="(v, i) in tokenNames" :value="v">
            {{ v }}
            {{
              balances[i] &&
              (["renCrv", "sbtcCrv", "hbtcCrv"].includes(v) ? "₿" : "$") +
                toFixed((balances[i] * virtual_prices[i]) / 1e36, v)
            }}
          </option>
        </select>
        ...
        <div class="flex-breakc noheight"></div>
        <button id="submit" @click="pay" :disabled="!isValidAddress">
          Pay
        </button>
      </div>
      <gas-price></gas-price>
...
import { getters, contract, gas as contractGas } from "../contract";
...
export default {
  ...
  computed: {
    ...
    crvAmount() {
      ...
      let maxAmount = this.toFixed(
        BN(this.amount).div(BN(this.virtual_prices[index]).div(1e18)),
        this.token
      );
      return maxAmount;
    },
    abis() {
      return Object.keys(allabis).filter(
        (pool) => pool != "susd" && pool != "y" && pool != "tbtc"
      );
    },
    isValidAddress() {
  ...
  created() {
    this.$watch(
      () => contract.default_account && contract.multicall,
      (val) => {
        if (!val) return;
        this.mounted();
      }
    ...
    if (poolParam) {
      let tokenIndex = Object.keys(allabis)
  ...
  methods: {
    async mounted() {
      this.contracts = this.abis.map(
        (pool) =>
          new contract.web3.eth.Contract(ERC20_abi, allabis[pool].token_address)
      );
      this.swaps = this.abis.map(
        (pool) =>
    ...
    async unstake(amount) {
      let waitingMessage = `Please confirm unstaking ${this.toFixed(
        amount,
        this.token
      )} ${this.token} tokens`;
      ...
      let rewardContract =
        this.token == "sCrv" ? this.rewards[0] : this.rewards[1];
    ...
    async pay() {
      let index = this.tokenNames.indexOf(this.token);
      let payAmount = BN(this.amount)
        .times(1e18)
        .div(BN(this.virtual_prices[index]))
        .times(1e18)
        .toFixed(0, 1);
      if (
        this.maxAmount.minus(BN(this.amount).times(1e36)).div(1e36).lt(BN(0.01))
      ) {
      ...
      if (["sCrv", "renCrv"].includes(this.token)) {
        ...
        if (payAmount.gt(BN(this.unstakedBalances[i])))
          await this.unstake(payAmount.minus(BN(this.unstakedBalances[i])));
      }
      await this.contracts[index].methods
        .transfer(this.to, payAmount.toFixed(0, 1))
        .send({
          from: contract.default_account,
          gasPrice: this.gasPriceWei,
          gas: 100000,
    ...
    async updateBalances() {
      let calls = this.abis.flatMap((pool, i) => {
        return [
          [
            allabis[pool].token_address,
            this.contracts[i].methods
              .balanceOf(contract.default_account)
              .encodeABI(),
          ],
          [
            allabis[pool].swap_address,
            this.swaps[i].methods.get_virtual_price().encodeABI(),
          ],
        ];
      });
      calls.push(
        ...this.rewards.map((reward) => [
          reward._address,
          reward.methods.balanceOf(contract.default_account).encodeABI(),
        ])
      );
      ...
      Vue.set(
        ...
        6,
        BN(this.balances[6])
          .plus(decoded[decoded.length - 1])
          .toString()
      );
    },
    highlight_amount() {
      let index = this.tokenNames.indexOf(this.token);
      this.maxAmount = BN(this.balances[index]).times(
        BN(this.virtual_prices[index])
      );
      if (BN(this.amount).times(BN(1e36)).gt(this.maxAmount))
        this.bgColor = "red";
