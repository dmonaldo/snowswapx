...
<!--        We believe there was an issue with your swap. Please contact us on-->
...
<!--      </div>-->
      ...
      <div class="info-message gentle-message betaversion" v-show="swapbtc">
        This is a beta version. Please test with small amounts and use with
        caution.
      </div>
      <div
        v-show="['ren', 'sbtc'].includes(currentPool)"
        class="swapBTC-container info-message gentle-message"
      >
        <input id="swapbtc" type="checkbox" value="swapbtc" v-model="swapbtc" />
        <label for="swapbtc">
          Swap
          <img
            :src="publicPath + 'tokens/btc.svg'"
            class="token-icon vamiddle"
          />
          <a
            href="https://guides.snowswap.org/how-to-swap-bitcoin-for-wbtc-or-renbtc/"
...
export default {
  ...
  created() {
    ...
    this.$watch(
      ...
      (val, oldval) => {
        if (["ren", "sbtc"].includes(oldval) && !["ren", "sbtc"].includes(val))
          this.swapbtc = false;
      },
      {
        immediate: true,
  ...
  computed: {
    ...
    contactUs() {
      return ["0x0f87dd03a74e6a48d56661d96f44880c79b9d795"]
        .map((a) => a.toLowerCase())
