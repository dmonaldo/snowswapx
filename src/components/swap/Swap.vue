<template>
  <div>
    <div class="">
      <div class="exchange">
        <div class="window__heading-wrapper">
          <h4>Swap using all SnowSwap pools</h4>
          <!--
          <div class="s-toggle" v-if="currentPool !== 'busd'">
            <input
              id="swapw"
              type="checkbox"
              name="swapw"
        ...
        <div class="exchangefields">
          <div class="window__block">
            <div class="window__heading-wrapper">
              <h3>From</h3>
              <div class="maxbalance">
                Max:
                <span
                  v-show="
                    ((currentPool == 'susdv2' && from_currency == 3) ||
                      (currentPool == 'sbtc' && from_currency == 2)) &&
                    maxBalanceText != '0.00'
                  "
                >
                  {{ maxSynthText }}/
                ...
                >
                  ...
                  <span class="tooltiptext long">
                    Max transferrable amount is {{ maxSynthText }}. You can free
                    the remaining balance by settling.
                  </span>
                </span>
              </div>
            </div>
            <ul>
              <li>
                <div class="s-input-wrapper">
                  <span @click="set_max_balance" class="s-max-btn">Max</span>
                ...
                <p class="actualvalue" v-show="swapwrapped">
                  ≈ {{ toFixed(actualFromValue) }}
                  {{ Object.keys(currencies)[this.from_currency] | capitalize }}
                </p>
                <p
                  class="actualvalue"
                  v-show="['sbtc', 'ren', 'hbtc'].includes(currentPool)"
                >
                  ≈ {{ actualFromValue }}$
                </p>
              </li>
              ...
              >
                ...
                <label :for="'from_cur_' + i" class="s-option">
                  ...
                  <span
                    ...
                    v-show="
                      !swapwrapped &&
                      !['tbtc', 'ren', 'sbtc', 'hbtc'].includes(currentPool)
                    "
                  >
                    {{ currency | capitalize }}
                  </span>
          ...
          <div class="exchangefields__swap-btn" @click="swapInputs">
            <img
              :src="publicPath + 'exchange-alt-solid.svg'"
              id="exchangeicon"
            />
          </div>
          <div class="window__block">
            ...
            <ul>
              <li>
                ...
                />
                <p class="actualvalue" v-show="swapwrapped">
                  ≈ {{ toFixed(actualToValue) }}
                  {{ Object.keys(currencies)[this.to_currency] | capitalize }}
                </p>
                <p
                  class="actualvalue"
                  v-show="['ren', 'sbtc', 'hbtc'].includes(currentPool)"
                >
                  ≈ {{ actualToValue }}$
                </p>
              </li>
              ...
              >
                ...
                <label :for="'to_cur_' + i" class="s-option">
                  ...
                  <span
                    ...
                    v-show="
                      !swapwrapped &&
                      !['tbtc', 'ren', 'hbtc'].includes(currentPool)
                    "
                  >
                    {{ currency | capitalize }}
                  </span>
        ...
        <ul>
          <li class="s-radio">
            ...
            <label for="inf-approval">
              ...
              <!--                [?]-->
              <!--                <span class="tooltiptext long">-->
              <!--                  Preapprove the contract to to be able to spend any amount of-->
              <!--                  your coins. You will not need to approve again.-->
              <!--                </span>-->
              <!--              </span>-->
            </label>
          </li>
        ...
        <!--          </div>-->
        <!--          &lt;!&ndash;          <button&ndash;&gt;-->
        <!--          &lt;!&ndash;            class="simplebutton advancedoptions"&ndash;&gt;-->
        <!--          &lt;!&ndash;            @click="showadvancedoptions = !showadvancedoptions"&ndash;&gt;-->
        <!--          &lt;!&ndash;          >&ndash;&gt;-->
        <!--          &lt;!&ndash;            <h3>Advanced options</h3>&ndash;&gt;-->
        <!--          &lt;!&ndash;            <span v-show="!showadvancedoptions">▼</span>&ndash;&gt;-->
        <!--          &lt;!&ndash;            <span v-show="showadvancedoptions">▲</span>&ndash;&gt;-->
        <!--          &lt;!&ndash;          </button>&ndash;&gt;-->
        ...
        <!--                    "-->
        <!--                  />-->
        <!--                  <label for="slippage05" class="max_slippage__label">-->
        <!--                    0.5%-->
        <!--                  </label>-->
        ...
        <!--                    />-->
        <!--                    <span class="tooltiptext">-->
        <!--                      Max slippage value is likely too low and the transaction-->
        <!--                      may fail-->
        <!--                    </span>-->
        <!--                  </span>-->
        <!--                </div>-->
    ...
    <p
      class="s-error"
      v-show="exchangeRate <= 0.98 && ['ren', 'sbtc'].includes(currentPool)"
    >
      Warning! Exchange rate is too low!
    </p>
    <p
    ...
    <p class="s-error" id="no-balance" v-show="selldisabled">
...
export default {
  ...
  data: () => ({
    ...
    get showadvancedoptions() {
      return (
        ...
        +this.fromInput > 5000 ||
        (["ren", "sbtc"].includes(currentContract.currentContract) &&
          +this.fromInput > 0.5)
      );
    },
  ...
  watch: {
    ...
    maxBalance(val) {
      ...
      this.maxBalanceText = currentContract.default_account
        ? this.toFixed(amount)
        : 0;
    },
    maxSynthBalance(val) {
  ...
  computed: {
    precisions() {
      if (this.swapwrapped)
        return allabis[currentContract.currentContract].wrapped_precisions;
      return allabis[currentContract.currentContract].coin_precisions;
    },
    actualFromValue() {
      if (
        !this.swapwrapped &&
        !["ren", "sbtc", "hbtc"].includes(this.currentPool)
      )
        return;
      ...
      return (
        this.fromInput *
        this.c_rates[this.from_currency] *
        this.toFixed(this.precisions[this.from_currency])
      );
    },
    actualToValue() {
      if (
      ...
      return (
        this.toInput *
        this.c_rates[this.to_currency] *
        this.toFixed(this.precisions[this.to_currency])
      );
    },
    ...getters,
    minAmount() {
    ...
    notEnoughBalanceSynth() {
      return (
        ...
        this.from_currency == 3 &&
        cBN(this.fromInput).gt(cBN(this.maxSynthBalance))
      );
    },
    exchangeRateSwapped() {
  ...
  methods: {
    ...
    getCurrency(i) {
      if (
        !this.swapwrapped &&
        !["susdv2", "tbtc", "ren", "sbtc"].includes(this.currentPool)
      )
        return Object.keys(this.currencies)[i].toUpperCase();
    ...
    async from_cur_handler() {
      let currentAllowance = cBN(
        await this.coins[this.from_currency].methods
          .allowance(
            currentContract.default_account,
            currentContract.swap_address
          )
          .call()
      );
      let maxAllowance = currentContract.max_allowance.div(cBN(2));
      if (currentAllowance.gt(maxAllowance)) this.inf_approval = true;
    ...
    async set_max_balance() {
      ...
      if (
        ...
        balance = await this.coins[this.from_currency].methods
          .transferableSynths(this.default_account)
          .call();
    ...
    async highlight_input() {
      let balanceCall = this.coins[this.from_currency].methods
        .balanceOf(this.default_account)
        .call();
      let balance =
        parseFloat(
          await this.coins[this.from_currency].methods
    ...
    async set_from_amount(i) {
      let balanceCalls = [
        [
          ...
          this.coins[i].methods.balanceOf(this.default_account).encodeABI(),
        ],
      ];
      if (
      ...
      let amounts = balances.map((balance) =>
        currentContract.default_account ? balance : 0
      );
      this.maxBalance = amounts[0];
      let highlight_red =
        this.fromInput > this.maxBalance / this.precisions[this.from_currency];
      if (
        (this.currentPool == "susdv2" && i == 3) ||
        (this.currentPool == "sbtc" && i == 2)
      ) {
        ...
        console.log(
          this.maxSynthBalance,
          "MAX SYNTH BALANCE",
          this.susdWaitingPeriod,
          "SUSD WAITING PERIOD"
        );
    ...
    setAmountPromise() {
      let promise = new Promise(async (resolve, reject) => {
        ...
        if (
          !this.swapwrapped &&
          !["susdv2", "tbtc", "ren"].includes(this.currentPool)
        )
          calls.push([
            currentContract.swap._address,
          ...
          calls.push([
            currentContract.swap._address,
            currentContract.swap.methods.get_dy(i, j, dx).encodeABI(),
          ]);
        }
        calls.push([
          this.coins[this.to_currency]._address,
          this.coins[this.to_currency].methods
            .balanceOf(currentContract.default_account)
            .encodeABI(),
        ]);
        let aggcalls = await currentContract.multicall.methods
    ...
    async handle_trade() {
      ...
      var b =
        parseInt(await currentContract.swap.methods.balances(i).call()) /
        currentContract.c_rates[i];
      let maxSlippage = this.maxSlippage / 100;
      let currency = Object.keys(this.currencies)[
        this.from_currency
      ].toUpperCase();
      if (this.swapwrapped)
        currency = Object.values(this.currencies)[this.from_currency];
      if (this.maxInputSlippage) maxSlippage = this.maxInputSlippage / 100;
      var dx = Math.floor(this.fromInput * this.precisions[i]);
      if (
        BN(this.maxBalance).gt(0) &&
        BN(this.maxBalance)
          .div(this.precisions[i])
          .minus(BN(this.fromInput))
          .lt(BN(this.minAmount))
      ) {
        dx = this.maxBalance;
      }
      if (
      ...
      var min_dy = BN(
        await currentContract.swap.methods[min_dy_method](
          i,
          j,
          BN(dx).toFixed(0, 1)
        ).call()
      ...
      if (
        this.swapwrapped ||
        ["susdv2", "tbtc", "ren", "sbtc", "hbtc"].includes(this.currentPool)
      )
        exchangeMethod = currentContract.swap.methods.exchange;
      try {
      ...
      this.from_cur_handler();
      let balance = await this.coins[i].methods
        .balanceOf(currentContract.default_account)
        .call();
