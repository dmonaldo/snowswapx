  ...
  methods: {
    async mounted() {
      ...
      for (let result of results) {
        if (!result) continue;
        let pair =
          result.returnValues.sold_id + "-" + result.returnValues.bought_id;
        ...
        if (
          ...
          this.gas[result.address][result.event] = {};
        if (!this.gas[result.address][result.event][pair])
          this.gas[result.address][result.event][pair] = [];
    ...
    async getEvents(fromBlock, toBlock) {
      return this.pools.map((pool) => {
        ...
        if (["tbtc", "ren"].includes(pool)) {
          topics = [
            this.swapContracts[this.pools.indexOf(pool)].getPastEvents(
