<template>
  <div>
    <div :class="{ 'window white': !pool }">
      <fieldset>
        <legend>
          Average liquidity provider profit [
          ...
          <span class="tooltip">
            [?]
            <span class="tooltiptext">(on annual basis)</span>
          </span>
          ]
        </legend>
      ...
      <div class="allapys">
        ...
        <p v-show="pool != 'susd'">
          ...
          <span :class="{ 'loading line': volumeData[0] < 0 }">
            <span v-show="volumeData[0] != -1">
              {{ volumeData[0] | formatNumber(0) }}$
            </span>
          </span>
        </p>
        <p v-show="['tbtc', 'ren', 'sbtc', 'hbtc'].includes(getPool)">
          Daily ₿ trading volume:
          <span :class="{ 'loading line': volumeData[0] < 0 }">
            <span v-show="volumeData[1] != -1">
              {{ volumeData[1] | formatNumber(8) }} ₿
            </span>
          </span>
        </p>
        <p>
        ...
        <p>
          ...
          <span class="tooltip">
            ...
            <span class="tooltiptext">
              Total APY since the pool was launched
            </span>
          </span>
        </p>
      </div>
  ...
  methods: {
    async mounted() {
      ...
      if (subdomain == "synthetix") {
        let res = await fetch(`https://${subdomain}.snowswap.org/stats.json`);
        ...
        for (let i = 0; i < data.length; i++) {
          if ((i % step_size == 0) | (i == data.length - 1)) {
            var el = data[i];
            chartData.push([el[0] * 1000, (el[1] / start_profit - 1) * 100]);
          }
        }
      } else {
        ...
        let newdata = await fetch(
          `${window.domain}/raw-stats/${subdomain}-${period}m.json`
        );
        ...
        let real_profit_0 = this.calcRealProfit(
    ...
    calcRealProfit(point, subdomain) {
      ...
      let real_profit = point.balances
        .map((balance, i) => {
          ...
          return (
            (real_amount / S) *
            (point.virtual_price / this.virtual_price_0) *
            price
          );
        })
        .reduce((a, b) => a + b, 0);
