    ...
    <fieldset>
      ...
      <div id="poolselect">
        <select class="tvision" v-model="event">
          <option v-for="(option, i) in allEvents" :value="i">
            {{ option }}
          </option>
        </select>
        <div id="poollist">
            ...
            <tr v-for="event in paginatedExchanges">
              ...
              <td>
                <a :href="`https://etherscan.io/tx/${event.transactionHash}`">
                  {{
                    isBTC(event)
                      ? event.soldAmount.toFixed(8)
                      : event.soldAmount.toFixed(2)
                  }}
                </a>
              </td>
              <td>
                <a :href="`https://etherscan.io/tx/${event.transactionHash}`">
                  {{
                    isBTC(event)
                      ? event.boughtAmount.toFixed(8)
                      : event.boughtAmount.toFixed(2)
                  }}
                </a>
              </td>
              <td>
          ...
          <tbody>
            ...
            <tr v-for="event in paginatedExchanges">
              ...
              <td>
                <a :href="`https://etherscan.io/tx/${event.transactionHash}`">
                  {{
                    event.returnValues.token_supply
        ...
        <div>
          <button
            id="loadmore"
            @click="loadMore"
            v-show="page == pages && exchanges.length > perPage"
          >
            Load more
          </button>
        </div>
  ...
  computed: {
    ...
    allAddresses() {
      return Object.keys(allabis)
        ...
        .map((pool) => ({ pool, address: allabis[pool].swap_address }));
    },
    addresses() {
      return Object.keys(allabis)
        .filter((pool) => this.pools.includes(pool))
        .map((pool) => allabis[pool].swap_address);
    },
    tokenExchangeEvent() {
    ...
    pages() {
      return (
        ...
        Math.ceil(this.exchanges.length / this.perPage) - 1
      );
    },
    changePagination() {
  ...
  methods: {
        ...
        .map(
          (pool) =>
            new web3.eth.Contract(
              allabis[pool].swap_abi,
              allabis[pool].swap_address
            )
        );
      ...
      return this.pools.map((pool) => {
        ...
        if (this.event == 2) {
          topics = this.removeLiquidityTopics[pool].map((topic) => {
            return this.swapContracts[
              this.allPools.indexOf(pool)
            ].getPastEvents(topic, {
              fromBlock: (block - numBlocks) | 0,
              toBlock: block,
            });
    ...
    pushState() {
      history.pushState(
        {},
        null,
        "/events/" + this.pools.join("_") + "/" + this.allEvents[this.event]
      );
    },
    selectPoolsHandler() {
      this.exchanges = [];
    ...
    async loadEvents(block) {
      ...
      while (length < 500) {
        ...
        while (true) {
          try {
            results = await Promise.all(
              this.getEvents(block, numBlocks).flat()
            );
            break;
          } catch (err) {
            console.error(err);
        ...
        results = results.flat().sort((a, b) => b.blockNumber - a.blockNumber);
        this.exchanges.push(...results);
        if (this.paginatedExchanges.length == 0 && this.perPage <= length)
          this.paginate();
        i++;
    ...
    async paginate() {
      ...
      if (this.event == 1 || this.event == 2) {
        this.paginatedExchanges = await Promise.all(
          exchanges.map((event) => {
      ...
      for (let event of exchanges) {
        ...
        if (prevEvent === undefined) {
          let pool = this.allAddresses.find(
            (v) => v.address.toLowerCase() == event.address.toLowerCase()
          ).pool;
          let poolIdx = this.allPools.indexOf(pool);
          try {
            virtual_price = await this.swapContracts[poolIdx].methods
              .get_virtual_price()
            ...
            console.error(err);
            virtual_price =
              this.findClosest(event.timestamp, poolIdx).virtual_price / 1e18;
          }
        } else {
    ...
    async selectPools() {
      ...
      this.exchanges = [];
      //get historic rates
      let fetchpools = this.pools.map((pool) =>
        pool == "iearn"
          ? "y"
          : pool == "susdv2"
          ? "susd"
      ...
      let requests = await Promise.all(
        fetchpools.map((pool) =>
          fetch(`${window.domain}/raw-stats/${pool}-1m.json`)
        )
      );
      ...
      for (let [i, data] of jsons.entries()) {
        ...
        this.rates[pool == "y" ? "iearn" : pool == "susd" ? "susdv2" : pool] =
          data[data.length - 1].rates;
      }
      this.subscriptions = [];
      this.page = 0;
      this.gotopage = 0;
      ...
      for (let [i, pool] of pools.entries()) {
        ...
        if (["susdv2", "tbtc", "ren", "sbtc"].includes(pool)) continue;
          ...
          for (let j = 0; j < 2; j++) {
            ...
            let calcRate =
              rate * (1 + (supplyRate * (block - oldBlock)) / 1e18);
    ...
    isBTC(event) {
      return [
        allabis.tbtc.swap_address.toLowerCase(),
        allabis.ren.swap_address.toLowerCase(),
      ].includes(event.address.toLowerCase());
    },
    getCurrency(event, type) {
      //type == 0 for sold
      //type == 1 for bought
      let i =
        type == 0 ? event.returnValues.sold_id : event.returnValues.bought_id;
      let contractAddress = event.address;
      let pool = this.allAddresses.find(
        (v) => v.address.toLowerCase() == contractAddress.toLowerCase()
      ).pool;
      if (event.event == "TokenExchange")
        return Object.values(allCurrencies[pool])[i];
    ...
    async formatEvent(event) {
      ...
      if (event.blockNumber > this.latestblock - 120) {
        ...
        virtual_price =
          (await this.swapContracts[poolIdx].methods
            .get_virtual_price()
            .call(null, event.blockNumber)) / 1e18;
      } else {
    ...
    subscribeExchange(event) {
      if (
        this.exchanges.findIndex(
          (prevEvent) => prevEvent.transactionHash == event.transactionHash
        ) !== -1
      )
        return;
        ...
        if (this.event == 0) {
          if (!["tbtc", "ren", "sbtc"].includes(pool)) {
            this.subscriptions.push(
              this.swapContracts[this.allPools.indexOf(pool)].events
    ...
    interpolatePoint(timestamp, poolidx) {
      ...
      if (timestamp > poolData[poolData.length - 1].timestamp)
        return poolData[poolData.length - 1];
      let next = poolData.findIndex(
      ...
      let point = {};
      let interpolator = helpers.interpolate(
        timestamp,
        prev.timestamp,
        next.timestamp
      );
    ...
    formatAddress(address) {
      if (
        address.toLowerCase() ==
        "0xeB21209ae4C2c9FF2a86ACA31E123764A3B6Bc06".toLowerCase()
      )
        return "Compound zap";
      if (
        address.toLowerCase() ==
        "0xac795D2c97e60DF6a99ff1c814727302fD747a80".toLowerCase()
      )
        return "usdt zap";
      if (
        address.toLowerCase() ==
        "0xbBC81d23Ea2c3ec7e56D39296F0cbB648873a5d3".toLowerCase()
      )
        return "y zap";
      if (
        address.toLowerCase() ==
        "0xb6c057591E073249F2D9D88Ba59a46CFC9B59EdB".toLowerCase()
      )
        return "busd zap";
      if (
        address.toLowerCase() ==
        "0xFCBa3E75865d2d561BE8D220616520c171F12851".toLowerCase()
      )
        return "susd zap";
      if (
        address.toLowerCase() ==
        "0xA50cCc70b6a011CffDdf45057E39679379187287".toLowerCase()
      )
        return "pax zap";
      if (
        address.toLowerCase() ==
        "0x9fe350dfa5f66bc086243f21a8f0932514316627".toLowerCase()
      )
        return "Curve Ren Adapter";
      return address.slice(0, 6) + "..." + address.slice(-6);
    },
    totalAmount(event) {
      if (event.returnValues.token_amount)
        return +event.returnValues.token_amount / 1e18;
    ...
    showAmounts(event) {
      ...
      let currencies = Object.entries(
        allCurrencies[pool == "y" ? "iearn" : pool == "susd" ? "susdv2" : pool]
      );
      return amounts.map(
        (amount, i) =>
          `${(+amount).toFixed(2)} ${currencies[i][0].toUpperCase()}`
      );
    },
    async formatAmounts(event) {
      if (event.returnValues.token_amount) {
        event.returnValues.token_amounts =
          event.returnValues.token_amount / 1e18;
      ...
      if (event.blockNumber > this.latestblock - 120) {
        ...
        rates = poolInfo.rates.map(
          (rate, i) => rate / 1e18 / allabis[pool].coin_precisions[i]
        );
      }
      event.returnValues.token_amounts = event.returnValues.token_amounts.map(
        (amount, i) => amount * rates[i]
      );
      event.formatted = true;
      return event;
    },
    formatAmount(event, type, poolInfo) {
      ...
      let amount =
        type == 0
          ? event.returnValues.tokens_sold
          : event.returnValues.tokens_bought;
