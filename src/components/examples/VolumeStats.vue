...
export default {
  ...
  async created() {
    ...
    let data = await Promise.all(
      pools.map((pool) =>
        fetch(`https://beta.snowswap.org/raw-stats/${pool}-30m.json`)
      )
    );
    this.data = await Promise.all(data.map((d) => d.json()));

    let btcPrices = await fetch(
        ...
        d.volume = Object.fromEntries(
          Object.entries(d.volume).map(([k, v]) => [
            ...
            v.map(
              (vol) =>
                vol *
                volumeStore.findClosestPrice(d.timestamp, btcPrices.prices)
            ),
          ])
        );
        return d;
      ...
      for (let [i, pooldata] of data.entries()) {
        ...
        for (let d of pooldata) {
          volume += Object.entries(d.volume)
            .map(
              ([pair, value]) =>
                value[0] / allabis[thepool].coin_precisions[pair.split("-")[0]]
            )
            .reduce((a, b) => a + b, 0);
        }
      }
        ...
        for (let d of pooldata) {
          ...
          volume += Object.entries(d.volume)
            ...
            .map(
              ([pair, value]) =>
                value[inverse ? 1 : 0] /
                allabis[thepool].coin_precisions[
                  pair.split("-")[inverse ? 1 : 0]
                ]
            )
            .reduce((a, b) => a + b, 0);
        }
