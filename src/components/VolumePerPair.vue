<template>
  <div>
    ...
    <div class="window white">
      ...
      <volume-per-coin-stats
        :data="pairVolumes && pairVolumes[selectpair]"
        :currency="selectpair"
        :loaded="loaded"
        :embedded="true"
      ></volume-per-coin-stats>
...
export default {
    ...
    chartdata: {
      ...
      tooltip: {
        ...
        pointFormatter() {
          ...
          if (["TBTC", "WBTC", "HBTC", "renBTC"].includes(this.series.name))
  ...
  computed: {
    currencies() {
      let pairs = Object.values(allCurrencies)
        .filter((p) => p != "susd")
        .map((v) => Object.keys(v));
      let allPairs = [];
  ...
  methods: {
    async created() {
      ...
      let filteredData = Object.keys(data)
        .filter((pool) => this.piepools.includes(pool))
        .reduce((obj, key) => ({ ...obj, [key]: data[key] }), {});

      pools = Object.entries(filteredData);
      this.pairVolumes = await volumeWorker.getVolumePerPair(
        filteredData,
        pools,
        allabis
      );
    ...
    async mounted() {
      while (this.chart && this.chart.series[0]) this.chart.series[0].remove();
    ...
    loadPieChart() {
      ...
      this.piechart.update(
        {
          title: {
            text: `Share of trading volume per pair for ${
              this.period != "all" ? "last " + this.period : "all time"
            }`,
          },
        },
        false
      ...
      let piechartdata = [];
      filtered.forEach((vol, i, arr) => {
        piechartdata.push({
          name: Object.keys(this.pairVolumes)[i],
          y: (vol / arr.reduce((a, b) => a + b, 0)) * 100,
        });
      });
      let highest = piechartdata
        .map((data) => data.y)
        .indexOf(Math.max(...piechartdata.map((data) => data.y)));
