  ...
  <div>
    <div class="swap exchange">
      <div class="exchangefields">
        <div class="window__block">
          <div class="window__heading-wrapper">
            ...
            <div
              class="maxbalance"
              :class="{ 'loading line': maxBalance == -1 }"
              @click="set_max_balance"
            >
              Max:
              <span
                v-show="maxSynthBalance != -1 && [5, 9].includes(from_currency)"
              >
                {{ maxSynthBalanceText }} /
              </span>
              ...
              <span v-show="[5, 9].includes(from_currency)" class="tooltip">
                [?]
                <span class="tooltiptext">
                  Max transferrable amount is {{ maxSynthBalanceText }}. You can
                  free the remaining balance by settling.
                </span>
              </span>
            </div>
          </div>
          <ul>
            <li>
              ...
              <p class="actualvalue" v-show="swapwrapped">
                ≈ {{ actualFromValue }}
                {{ Object.keys(currencies)[this.from_currency] | capitalize }}
              </p>
            </li>
            <li
              :class="{ coins: true, [currency]: true }"
            ...
            >
              ...
              />
              <label :for="'from_cur_' + i" class="s-option">
                <img
                  :class="{
                    'token-icon': true,
                  }"
                ...
                />
                <span v-show="!swapwrapped" class="currency-symbol">
                  {{ currency | capitalize }}
                </span>
                <span v-show="swapwrapped" class="currency-symbol">
                  {{ currencies[currency] }}
        ...
        <div class="exchangefields__swap-btn" @click="swapInputs">
          <img :src="publicPath + 'exchange-alt-solid.svg'" id="exchangeicon" />
        </div>
        <div class="window__block">
          <div class="window__heading-wrapper">
            <h3>To</h3>
            <div class="maxbalance2">
              Max:
              <span></span>
            </div>
          </div>
          <ul>
            <li>
              ...
              <p class="actualvalue" v-show="swapwrapped">
                ≈ {{ actualToValue }}
                {{ Object.keys(currencies)[this.to_currency] | capitalize }}
              </p>
            </li>
            <li
              :class="{ coins: true, [currency]: true }"
            ...
            >
              <input
                ...
                v-model="to_currency"
              />
              <label :for="'to_cur_' + i" class="s-option">
                <img
                  :class="{
                    'token-icon': true,
                  }"
      ...
      <p v-show="fromInput > 0" class="exchange-rate">
        Exchange rate
        <span @click="swapExchangeRate" class="exchange-rate_swap-rates">
          {{ getPair(swaprate) }}
        ...
        <span
          id="exchange-rate"
          @click="swapExchangeRate"
          class="exchange-rate_swapped"
        >
          {{ exchangeRateSwapped }}
        </span>
      </p>
      <p v-show="fromInput > 0" class="exchange-rate">
        ...
        <span id="best-pool" class="exchange-rate_swapped">
      ...
      <ul>
        <li v-show="bestPool !== null" class="s-radio">
          <input
            id="inf-approval"
            type="checkbox"
            name="inf-approval"
          ...
          <label for="inf-approval">
            Infinite approval - trust {{ bestPoolText }} contract forever
          </label>
        </li>
      ...
      <div>
        <div class="advancedoptions">
          <h3 @click="showadvancedoptions = !showadvancedoptions">
            Advanced options
          </h3>
          ...
          <div>
            <div id="poolselect">
              ...
              <input
                ...
                v-model="pools"
              />
              <label for="compoundpool1" class="s-option">Compound</label>

              <input id="ypool1" type="checkbox" value="y" v-model="pools" />
              <label for="ypool1" class="s-option">Y</label>
              ...
              <!--                v-model="pools"-->
              <!--              />-->
              <!--              <label for="busdpool1" class="s-option">bUSD</label>-->
              ...
              <!--                v-model="pools"-->
              <!--              />-->
              <!--              <label for="susdpool1" class="s-option">sUSD</label>-->
              ...
              <!--                v-model="pools"-->
              <!--              />-->
              <!--              <label for="paxpool1" class="s-option">PAX</label>-->
              ...
              <!--                v-model="pools"-->
              <!--              />-->
              <!--              <label for="renpool1" class="s-option">ren</label>-->
              ...
              <!--                v-model="pools"-->
              <!--              />-->
              <!--              <label for="sbtcpool" class="s-option">sBTC</label>-->
              ...
              <!--                v-model="pools"-->
              <!--              />-->
              <!--              <label for="hbtcpool" class="s-option">HBTC</label>-->
            </div>
            <div v-show="fromInput > 0" id="max_slippage">
              <div class="max_slippage__content">
                ...
                <input
                  ...
                  "
                />
                <label for="slippage05" class="max_slippage__label">0.5%</label>
                ...
                <input
                  ...
                  "
                />
                <label for="slippage1" class="max_slippage__label">1%</label>
                ...
                <label
                  for="custom_slippage"
                  @click="customSlippageDisabled = false"
                  class="max_slippage__label"
                >
                  <input
                    type="text"
                ...
                <span class="tooltip" v-show="showSlippageTooLow">
                  <img
                    class="icon small hoverpointer warning"
                    :src="publicPath + 'exclamation-circle-solid.svg'"
                  />
                  <span class="tooltiptext">
                    Max slippage value is likely too low and the transaction may
                    fail
                  </span>
                </span>
              </div>
      ...
      <p
        ...
        v-show="
          ...
          !susdWaitingPeriod &&
          +maxSynthBalanceText > 0
        "
      >
        Max balance you can use is {{ maxSynthBalanceText }}
      </p>
      <div class="simple-error pulse" v-show="susdWaitingPeriod">
        Cannot transfer {{ from_currency == 5 ? "sUSD" : "sBTC" }} during
        waiting period {{ susdWaitingPeriodTime.toFixed(0) }} secs left
      </div>
      ...
      <p class="trade-buttons">
        <button id="trade" @click="handle_trade" :disabled="selldisabled" class="s-button s-button--primary">
          Sell
          <span class="loading line" v-show="loadingAction"></span>
        </button>
      </p>
      <div
        class="info-message gentle-message waiting-message"
        v-show="show_loading"
      >
      ...
      <p class="simple-error" id="no-balance" v-show="showNoBalanceWarning">
        ...
        <span v-show="!swapwrapped">
          {{ Object.keys(currencies)[from_currency] | capitalize }}
        </span>
        <span v-show="swapwrapped">
          {{ Object.values(currencies)[from_currency] }}
        </span>
        .
        <span>Swap is not available.</span>
      </p>
      <div class="info-message gentle-message" v-show="selldisabled">
        Swapping between {{ Object.values(currencies)[from_currency] }} and
        {{ Object.values(currencies)[to_currency] }} is not available currently
      </div>
...
import contractAbis, {
  ...
  yERC20_abi,
  synthERC20_abi,
  synthetixExchanger_address,
  synthetixExchanger_ABI,
  onesplit_address,
  onesplit_abi,
} from "../../allabis";
...
export default {
  ...
  data: () => ({
    ...
    get showadvancedoptions() {
      return (
        localStorage.getItem("advancedoptions") === "true" ||
        +this.fromInput > 5000 ||
        ([7, 8, 9].includes(this.from_currency) && +this.fromInput > 0.5)
      );
    },
  ...
  computed: {
    ...
    CONTRACT_FLAG() {
      ...
      let addPoolFlag = Object.keys(curveFlags)
        .filter((pool) => this.pools.includes(pool))
        .map((pool) => curveFlags[pool]);
    ...
    maxBalanceText() {
      return this.toFixed(
        this.maxBalance / this.precisions(this.from_currency)
      );
    },
    maxSynthBalanceText() {
      if (isNaN(this.maxSynthBalance)) return "0.00";
      return this.toFixed(
        this.maxSynthBalance / this.precisions(this.from_currency)
      );
    },
    notEnoughBalanceSynth() {
      return (
        [5, 9].includes(this.from_currency) &&
        BN(this.fromInput).gt(
          BN(this.maxSynthBalance).div(this.precisions(this.from_currency))
        )
      );
    },
    actualFromValue() {
      ...
      return (
        this.fromInput *
        this.c_rates(this.from_currency)[this.from_currency] *
    ...
    actualToValue() {
      ...
      return (
        this.toInput *
        this.c_rates(this.to_currency)[this.to_currency] *
        this.precisions(this.to_currency)
      ).toFixed(2);
    },
    bestPoolText() {
    ...
    selldisabled() {
      ...
      if (
        (this.from_currency <= 6 && this.to_currency >= 7) ||
        (this.to_currency <= 6 && this.from_currency >= 7)
      )
        return true;
    ...
    warningNoPool() {
      ...
      if (
        ([7, 8].includes(this.from_currency) ||
          [7, 8].includes(this.to_currency)) &&
        !this.pools.includes("ren")
      ) {
        poolMessage = "ren";
      }
      if (
        (this.from_currency == 6 || this.to_currency == 6) &&
        !this.pools.includes("pax")
      ) {
        poolMessage = "pax";
      }
      if (
        (this.from_currency == 5 || this.to_currency == 5) &&
        !this.pools.includes("susdv2")
      ) {
        poolMessage = "susd";
      }
      if (
        (this.from_currency == 4 || this.to_currency == 4) &&
        !this.pools.includes("busd")
      ) {
        poolMessage = "busd";
      }
      if (
        (this.from_currency == 3 || this.to_currency == 3) &&
        !this.pools.includes("y")
      ) {
        poolMessage = "y";
      }
      if (
        (this.from_currency == 2 || this.to_currency == 2) &&
        this.pools.find((pool) =>
          ["usdt", "y", "busd", "susdv2", "pax"].includes(pool)
        ) == undefined
      ) {
        poolMessage = "usdt";
      }
    ...
    decodeDistribution() {
      ...
      //multipath USDC
      if (
        this.usedFlags == this.CONTRACT_FLAG - 0x10000 - 0x400000000 &&
        this.distribution.find((v) => +v > this.usedParts) !== undefined
      ) {
        this.multipath = 1;
      }
      //multipath DAI
      if (
        this.usedFlags == this.CONTRACT_FLAG - 0x20000 - 0x400000000 &&
        this.distribution.find((v) => +v > this.usedParts) !== undefined
      ) {
        this.multipath = 2;
      }
      //multipath USDT
      if (
        this.usedFlags == this.CONTRACT_FLAG - 0x10000 - 0x20000 &&
        this.distribution.find((v) => +v > this.usedParts) !== undefined
      ) {
        this.multipath = 3;
      }
    ...
    distributionText() {
      ...
      for (let j = this.decodeDistribution.length - 1; j >= 0; j--) {
        if (this.multipath > 0 && j == 1)
          text +=
            Object.values(this.currencies)[this.from_currency] +
            " -> " +
            multipaths[this.multipath - 1] +
            "<br>";
        ...
        if (this.multipath > 0 && j == 0)
          text +=
            multipaths[this.multipath - 1] +
            " -> " +
            Object.values(this.currencies)[this.to_currency] +
            "<br>";
        ...
        for (let [i, v] of this.decodeDistribution[j].entries()) {
          ...
          text +=
            "" +
            ((100 * v) / this.usedParts).toFixed(2) +
            "% " +
            distPools[i] +
            "<br>";
        }
    ...
    exchangeRateSwapped() {
      if (this.swaprate) return (1 / this.exchangeRate).toFixed(4);
      else return this.exchangeRate;
    },
    gasPrice() {
  ...
  async created() {
    ...
    let unwatch = this.$watch(
      () => contract.web3 && contract.multicall,
      (val) => {
        this.mounted();
        unwatch();
      }
  ...
  methods: {
    ...
    getPair(inverse = false) {
      let from = !this.swapwrapped
        ? Object.keys(this.currencies)[this.from_currency]
        : Object.values(this.currencies)[this.from_currency];
      let to = !this.swapwrapped
        ? Object.keys(this.currencies)[this.to_currency]
        : Object.values(this.currencies)[this.to_currency];
    ...
    handleError(err) {
      console.error(err);
      (this.waitingMessage = ""), (this.show_loading = false);
      throw err;
    },
    handleCheck(val) {
    ...
    c_rates(i) {
      if (this.swapwrapped == 2 && i < 3)
        return contract.contracts.iearn.c_rates;
      if (this.swapwrapped == 2 && i == 3)
        return contract.contracts.busd.c_rates;
      else return contract.c_rates;
    },
    getCoins(i) {
      if (this.swapwrapped == 1) return this.coins.slice(0, 2)[i];
      else if (this.swapwrapped == 2) return this.coins.slice(2)[i];
      else return this.underlying_coins[i];
    },
    normalizeCurrency(i) {
    ...
    precisions(i, contractName) {
      ...
      if (!contractName && this.swapwrapped == 2 && i < 3)
        contractName = "iearn";
      if (!contractName && this.swapwrapped == 2 && i == 3)
        contractName = "busd";
      if (!contractName) contractName = contract.currentContract;
      if (this.swapwrapped)
        return contractAbis[contractName].wrapped_precisions[i];
      return contractAbis[contractName].coin_precisions[i];
    },
    async highlight_input() {
      var balance =
        parseFloat(
          await this.getCoins(this.from_currency)
            .methods.balanceOf(
      ...
      if (this.fromInput > balance)
        this.fromBgColor = "rgba(250, 102, 127, 0.15)";
      else this.fromBgColor = "#F4F6FA";
    },
    ...
    async handle_trade() {
      ...
      let min_dy = BN(this.toInput)
        .times(this.precisions(j))
        .times(BN(1 - maxSlippage))
        .toFixed(0);
      let pool = contract.currentContract;
      let bestContract = contract;
      if (this.bestPool > 0 && this.bestPool < 7) {
        let poolIdx = this.bestPool;
        pool = Object.keys(contract.contracts).filter(
          (pool) => pool != "usdt" && pool != "tbtc"
        )[this.bestPool];
      ...
      this.waitingMessage = `Please approve ${
        this.fromInput
      } ${this.getCurrency(this.from_currency)} for exchange`;
      var { dismiss } = notifyNotification(this.waitingMessage);
      try {
        if (this.inf_approval)
          await common.ensure_underlying_allowance(
            this.from_currency,
            contract.max_allowance,
            this.underlying_coins,
            address,
            this.swapwrapped,
            bestContract
          );
      ...
      this.waitingMessage = `Please confirm swap
        ...
        this.from_currency
                                        ...
                                        for min ${this.toFixed(
                                          min_dy / this.precisions(j)
                                        )} ${this.getCurrency(
        this.to_currency
      )}`;
      var { dismiss } = notifyNotification(this.waitingMessage);
      if (this.bestPool == 7) {
        ...
        if (
          this.swapwrapped ||
          ["susd", "ren", "sbtc", "hbtc"].includes(this.bestPoolText)
        )
          exchangeMethod = bestContract.swap.methods.exchange;
        i = this.normalizeCurrency(i);
        j = this.normalizeCurrency(j);
        try {
          await exchangeMethod(i, j, amount, min_dy)
            .send({
              from: contract.default_account,
              gasPrice: this.gasPriceWei,
              gas: this.swapwrapped
                ? contractGas.swap[pool].exchange(i, j)
                : contractGas.swap[pool].exchange_underlying(i, j),
            })
            .once("transactionHash", (hash) => {
              notifyHandler(hash);
              this.waitingMessage = `Waiting for swap
        ...
        this.from_cur_handler();
        let balance = await this.getCoins(this.from_currency)
          .methods.balanceOf(
            contract.default_account ||
              "0x0000000000000000000000000000000000000000"
          )
    ...
    async set_from_amount(i) {
      ...
      if ([5, 9].includes(i)) {
        ...
        balanceCalls.push([
          this.snxExchanger._address,
          this.snxExchanger.methods
            .maxSecsLeftInWaitingPeriod(contract.default_account, currencyKey)
            .encodeABI(),
        ]);
      }
      let aggcalls = await contract.multicall.methods
        .aggregate(balanceCalls)
        .call();
      let balances = aggcalls[1].map((hex) =>
        contract.web3.eth.abi.decodeParameter("uint256", hex)
      );
      let amounts = balances.map((balance) =>
        contract.default_account ? balance : 0
      );
      this.maxBalance = amounts[0];
      let highlight_red =
        this.fromInput > balances[0] / this.precisions(this.from_currency);
      if ([5, 9].includes(i)) {
      ...
      if (highlight_red) this.fromBgColor = "rgba(250, 102, 127, 0.15)";
      else this.fromBgColor = "#F4F6FA";
    ...
    getCalls(amount) {
      let defaultCalls = [
        this.makeCall(
          amount,
          10,
          this.CONTRACT_FLAG - 0x10000 - 0x20000 - 0x400000000
        ),
      ];
      let calls = defaultCalls.concat();
      if (
        [3, 4, 5, 6].includes(this.from_currency) &&
        [3, 4, 5, 6].includes(this.to_currency)
      ) {
      ...
      if (
        ...
        calls.push(this.makeCall(amount, 1000, 4403952091136));
      }
      return calls;
    },
    async set_to_amount_onesplit() {
      let amount = BN(this.fromInput)
        .times(this.precisions(this.from_currency))
        .toFixed(0);
      let calls = this.getCalls(amount);
      this.swapPromise.cancel();
      let aggcalls = contract.multicall.methods
        .aggregate(calls)
        .call(undefined, "pending");
      this.swapPromise = helpers.makeCancelable(aggcalls);
      let split_swap = await this.swapPromise;
      let decoded = split_swap[1].map((hex) =>
        contract.web3.eth.abi.decodeParameters(["uint256", "uint256[]"], hex)
      );
      let max = decoded.reduce((a, b) => (a[0] > b[0] ? a : b));
      let decodedCall = contract.web3.eth.abi.decodeParameters(
      ...
      console.log(
        max,
        "1split swap",
        this.underlying_coins[this.from_currency],
        this.underlying_coins[this.to_currency]
      );

      let amount_dy = max[0];
      let exchangeRate = BN(amount_dy)
        .div(this.precisions(this.to_currency))
        .div(this.fromInput);
    ...
    setExchangeRate(exchangeRate) {
      if (+exchangeRate <= 0.98) this.bgColor = "rgba(250, 102, 127, 0.15)";
      else this.bgColor = "#F4F6FA";
    ...
    getPoolsCalls() {
      ...
      if (this.swapwrapped == 1) {
        pools = ["compound", "usdt"];
        let dx = BN(this.fromInput).times(
          contractAbis.usdt.wrapped_precisions[this.from_currency]
        );
        calls = [
          [
            this.swap[0]._address,
            this.swap[0].methods
              .get_dy(this.from_currency, this.to_currency, dx.toFixed(0, 1))
              .encodeABI(),
          ],
          [
      ...
      if (!this.swapwrapped) {
        let dx = BN(this.fromInput).times(
          contractAbis.iearn.coin_precisions[this.from_currency]
        );
        //TUSD only in y pool
        if (
          (this.from_currency == 3 || this.to_currency == 3) &&
        ...
        //BUSD only in b pool
          ...
          let dx = BN(this.fromInput).times(
            contractAbis.busd.coin_precisions[from_currency]
          );
          ...
          calls = [
            [
              ...
              this.swap[3].methods
                .get_dy_underlying(from_currency, to_currency, dx.toFixed(0, 1))
                .encodeABI(),
            ],
          ];
        }
        //sUSD exchanges only in s pool
          ...
          let dx = BN(this.fromInput).times(
            contractAbis.susdv2.coin_precisions[from_currency]
          );
          ...
          calls = [
            [
              ...
              this.swap[4].methods
                .get_dy(from_currency, to_currency, dx.toFixed(0, 1))
                .encodeABI(),
            ],
          ];
        }
        //PAX exchanges only in PAX pool
          ...
          let dx = BN(this.fromInput).times(
            contractAbis.pax.coin_precisions[from_currency]
          );
          calls = [
            [
          ...
          ([7, 8, 9].includes(this.from_currency) ||
            [7, 8, 9].includes(this.to_currency)) &&
          ...
          let dx = BN(this.fromInput).times(
            contractAbis.sbtc.coin_precisions[from_currency]
          );
          let poolidx = this.pools
            .filter((pool) => ["ren", "sbtc"].includes(pool))
            .map((pool) => this.allPools.indexOf(pool) + 1);
          if (this.from_currency == 9 || this.to_currency == 9)
            poolidx = [poolidx.filter((id) => id != 7)];
          calls = poolidx.map((i) => {
            return [
          ...
          [10].includes(this.from_currency) ||
          ([10].includes(this.to_currency) && this.pools.includes("hbtc"))
        ) {
          let from_currency = this.from_currency - 9;
          let to_currency = this.to_currency - 9;

          let dx = BN(this.fromInput).times(
            contractAbis.hbtc.coin_precisions[from_currency]
          );
          ...
          dx = BN(this.fromInput).times(
            contractAbis.usdt.coin_precisions[this.from_currency]
          );
          let poolidx = this.pools
            .filter((pool) => !["tbtc", "ren", "sbtc", "hbtc"].includes(pool))
            .map((pool) => this.allPools.indexOf(pool));
          if (this.from_currency == 2 || this.to_currency == 2)
            poolidx = poolidx.filter((id) => id != 0);
          calls = poolidx.map((i) => [
            this.swap[i]._address,
            this.swap[i].methods
              .get_dy_underlying(
    ...
    async set_to_amount() {
      ...
      try {
        ...
        if (this.from_currency == 10 || this.to_currency == 10) {
          let dx = BN(
            this.fromInput * this.precisions(this.from_currency)
          ).toFixed(0, 1);
          ...
          let dy = await this.swap[9].methods
            .get_dy(actualFromCurrency, actualToCurrency, dx)
            .call();
          this.bestPool = 8;
          ...
          let useOneSplit =
            this.fromInput * exchangeRate1 - this.fromInput * exchangeRate >
            txPrice1split - txPricePool;
        ...
        if (
          BN(
            await this.getCoins(this.from_currency)
              ...
              .call()
          ).gt(contract.max_allowance.div(BN(2)))
        )
          this.inf_approval = true;
        else this.inf_approval = false;
        ...
        if (this.swapwrapped) {
          let cdy_ =
            bestdy_ *
            this.c_rates(this.to_currency)[this.to_currency] *
            contractAbis.compound.wrapped_precisions[this.to_currency];
          let cdx_ =
            this.fromInput *
            this.c_rates(this.from_currency)[this.from_currency] *
            contractAbis.compound.wrapped_precisions[this.from_currency];
    ...
    async set_max_balance() {
      ...
      if ([5, 9].includes(this.from_currency)) {
        balance = await this.getCoins(this.from_currency)
          .methods.transferableSynths(
            contract.default_account ||
              "0x0000000000000000000000000000000000000000"
          )
        ...
        balance = await this.getCoins(this.from_currency)
          ...
          .call();
      let amount = BN(balance)
        .div(this.precisions(this.from_currency))
        .toFixed();
      this.fromInput = contract.default_account ? amount : 0;
      await this.set_to_amount();
    },
    async setup() {
      this.onesplit_address = await helpers.retry(
        contract.web3.eth.ens.getAddress("1split.eth")
      );
      this.onesplit = new contract.web3.eth.Contract(
        onesplit_abi,
        this.onesplit_address
      );
      this.swap = [];
      this.addresses = [];
      let abis = Object.keys(contractAbis).filter(
        (p) => p != "susd" && p != "y"
      );
      for (let pool of abis) {
        this.swap.push(
          new contract.web3.eth.Contract(
            contractAbis[pool].swap_abi,
            contractAbis[pool].swap_address
          )
        );
        this.addresses.push({
          address: contractAbis[pool].swap_address,
          pool: pool,
        });
      }
      ...
      this.coins.push(
        new contract.web3.eth.Contract(
          cERC20_abi,
          contractAbis.compound.coins[0]
        )
      );
      this.coins.push(
        new contract.web3.eth.Contract(
          cERC20_abi,
          contractAbis.compound.coins[1]
        )
      );
      for (let i = 0; i < 4; i++) {
        ...
        this.coins.push(
          new contract.web3.eth.Contract(
            yERC20_abi,
            contractAbis.iearn.coins[i]
          )
        );
        this.underlying_coins.push(
          new contract.web3.eth.Contract(
            ERC20_abi,
            contractAbis.iearn.underlying_coins[i]
          )
        );
      }
      ...
      //busd
      this.coins.push(
        new contract.web3.eth.Contract(yERC20_abi, contractAbis.busd.coins[3])
      );
      this.underlying_coins.push(
        new contract.web3.eth.Contract(
          ERC20_abi,
          contractAbis.busd.underlying_coins[3]
        )
      );
      ...
      this.coins.push(
        new contract.web3.eth.Contract(ERC20_abi, contractAbis.susdv2.coins[3])
      );
      this.underlying_coins.push(
        new contract.web3.eth.Contract(
          synthERC20_abi,
          contractAbis.susdv2.underlying_coins[3]
        )
      );

      //pax
      this.coins.push(
        new contract.web3.eth.Contract(yERC20_abi, contractAbis.pax.coins[3])
      );
      this.underlying_coins.push(
        new contract.web3.eth.Contract(
          ERC20_abi,
          contractAbis.pax.underlying_coins[3]
        )
      );

      for (let i = 0; i < contractAbis.ren.coins.length; i++) {
        this.coins.push(
          new contract.web3.eth.Contract(ERC20_abi, contractAbis.ren.coins[i])
        );
        this.underlying_coins.push(
          new contract.web3.eth.Contract(ERC20_abi, contractAbis.ren.coins[i])
        );
      ...
      //hbtc
      this.coins.push(
        new contract.web3.eth.Contract(ERC20_abi, contractAbis.hbtc.coins[1])
      );
      this.underlying_coins.push(
        new contract.web3.eth.Contract(ERC20_abi, contractAbis.hbtc.coins[1])
      );

      this.snxExchanger = new contract.web3.eth.Contract(
        synthetixExchanger_ABI,
        synthetixExchanger_address
      );
    },
...
.advancedoptions {
  margin-top: 40px;
  padding: 26px 0;
  border-top: 1px solid rgba(217, 223, 234, 0.5);
}
.trade-buttons {
  display: flex;
  justify-content: center;
