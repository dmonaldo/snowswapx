<template>
  <div class="tradeview window white">
    <fieldset id="onesplit" v-if="exchange">
      <legend class="text-center">Swap using all SnowSwap pools</legend>
      <one-split />
    </fieldset>
...
export default {
          ...
          series: {
            point: {
              events: {
                click: (function (self) {
                  return function (e) {
                    let nearest = self.chart.pointer.findNearestKDPoint(
                      self.chart.series,
                      false,
                      e
                    );

                    let index = nearest.dataGroup
                      ? nearest.dataGroup.start
                      : nearest.index;
                    let poolDataPoint = self.data
                      .map((p) => p.filter((v) => v.timestamp))
                      .map((p) => p[index])
                      .filter((p) => p);
  ...
  watch: {
    selectChange() {
      ...
      contract.allInitContracts.size >= tradeStore.pools.length &&
        this.mounted();
    },
  },
  mounted() {
    ...
    this.watchLoadedContracts();
    if (contract.allInitContracts.size >= tradeStore.pools.length)
      this.mounted();
  },
  beforeDestroy() {
  ...
  methods: {
    watchLoadedContracts() {
      let unwatch = this.$watch(
        () => contract.allInitContracts.size,
        (val) => {
    ...
    async loadData() {
      ...
      if (intervalIndex >= tradeStore.intervals.indexOf("30m"))
        jsonInterval = "30m";
      if (intervalIndex >= tradeStore.intervals.indexOf("1d"))
        jsonInterval = "1440m";
      let urls = tradeStore.pools.map((pool) => {
        let fetchpool =
          pool == "iearn"
            ...
            : pool;
        return fetch(
          `${window.domain}/raw-stats/${fetchpool}-${jsonInterval}.json`
        );
      });
      if (
        tradeStore.pools.includes("tbtc") ||
        tradeStore.pools.includes("ren") ||
        tradeStore.pools.includes("sbtc") ||
        tradeStore.pools.includes("hbtc")
      )
        urls.push(
          fetch(`
      ...
      for (let res of requests) {
        ...
        if (
          res.url.includes("tbtc") ||
          res.url.includes("ren") ||
          res.url.includes("sbtc") ||
          res.url.includes("hbtc")
        ) {
          json = json.map((d) => {
            d.volume = Object.fromEntries(
              Object.entries(d.volume).map(([k, v]) => [
                k,
                v.map(
                  (vol) =>
                    vol *
                    volumeStore.findClosestPrice(
                      d.timestamp,
                      this.btcPrices.prices
                    )
                ),
              ])
            );
    ...
    async loadRecentData(timestamp) {
      await this.loadData();
      let newData = this.data.map((json) =>
        json.filter((data) => data.timestamp > timestamp)
      );
      ...
      try {
        let lastPriceCalls = this.pools.map((pool) => {
          ...
          return [
            ...
            window[pool].swap.methods[get_method](
              this.fromCurrency,
              this.toCurrency,
              BN(amount)
                .times(abis[pool].coin_precisions[this.fromCurrency])
                .toFixed(0)
            ).encodeABI(),
          ];
        });
        let aggcalls = await contract.multicall.methods
          .aggregate(lastPriceCalls)
          .call();
        let lastPrices = aggcalls[1].map((hex) =>
        ...
        for (let l = length - 1; l > 0; l -= chunkSize) {
          for (let i = l; i > l - chunkSize && i > 0; i--) {
            ...
            for (let j = 0; j < data.length; j++) {
              ...
              let amount = 1;
              if (["tbtc", "ren", "sbtc", "hbtc"].includes(this.pools[j]))
                amount = 1 / 1e8;
              ...
              let get_dy_underlying = await calcWorker.calcPrice(
                { ...v, ...this.poolConfigs[j] },
                this.fromCurrency,
                this.toCurrency,
                amount * abis[this.pools[j]].coin_precisions[this.fromCurrency]
              );
              let calcprice = +BN(get_dy_underlying).div(
                amount * abis[this.pools[j]].coin_precisions[this.toCurrency]
              );
              if (calcprice == 0) continue;
              if (this.inverse) calcprice = 1 / calcprice;
              if (v.prices[this.pairIdx]) {
                if (this.inverse)
                  v.prices[this.pairIdx] = v.prices[this.pairIdx].map(
                    (price) => 1 / price
                  );
              ...
              if (i == length - 1) {
                let dx = BN(
                  abis[this.pools[j]].coin_precisions[this.fromCurrency]
                ).toFixed(0);
                let amount = 1;
                if (["tbtc", "ren", "sbtc", "hbtc"].includes(this.pools[j]))
                  amount = 1 / 1e4;
                let lastPrice = +BN(lastPrices[j]).div(
                  amount * abis[this.pools[j]].coin_precisions[this.toCurrency]
                );
                if (this.inverse) lastPrice = 1 / lastPrice;
                this.ohlcData[i].prices[this.pairIdx].push(lastPrice);
              }
              this.ohlcData[i].prices[this.pairIdx].push(
                ...v.prices[this.pairIdx]
              );
              this.ohlcData[i].volume[this.pairIdx][j] = v.volume[
                this.pairIdx
              ].map((v, k) => {
                if (k == 0)
                  return (
                    v / abis[this.pools[j]].coin_precisions[this.fromCurrency]
                  );
          ...
          for (let i = l; i > l - chunkSize && i > 0; i--) {
            ...
            let volumeData = this.ohlcData[i].volume[this.pairIdx].map(
              (vs) => vs[0]
            );
            if (this.inverse)
              volumeData = this.ohlcData[i].volume[this.pairIdx].map(
                (vs) => vs[1]
              );
            let volumePoint = [
              this.ohlcData[i].timestamp * 1000, // the date
    ...
    async mounted() {
      ...
      this.chart.rangeSelector.clickButton(
        tradeStore.intervals.indexOf(this.interval),
        true
      );
      this.chart.redraw();
      this.chart.hideLoading();
