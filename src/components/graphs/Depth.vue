  ...
  methods: {
    ...
    setExtremes() {
      ...
      this.chart.xAxis[0].setExtremes(
        this.chart.series[1].xData[0] - 0.001,
        this.chart.series[0].xData[0] + 0.001,
        true,
        true
      );
    },
    ...
    async mounted(lastTimestamp) {
      ...
      for (let [key, pool] of tradeStore.pools.entries()) {
        if (
          poolConfigs[key].N_COINS - 1 < toCurrency ||
          poolConfigs[key].N_COINS - 1 < fromCurrency
        )
          continue;
      ...
      let imax = Math.floor(
        100 * (1 + Math.log10(10) / Math.log10(balanceSum))
      );
      this.imax = imax;
      ...
      for (let i = 0; i <= imax; i++) {
        ...
        for (let j = 0; j < pools.length; j++) {
          if (
            poolConfigs[j].N_COINS - 1 < toCurrency ||
            poolConfigs[j].N_COINS - 1 < fromCurrency
          )
            continue;
          ...
          let dy = await calcWorker.calcPrice(
            { ...tradeStore.poolInfo[poolIdx[j]], ...poolConfigs[j] },
            fromCurrency,
            toCurrency,
            BN(dx1).toFixed(0),
            true
          ...
          let dx = await calcWorker.calcPrice(
            { ...tradeStore.poolInfo[poolIdx[j]], ...poolConfigs[j] },
            toCurrency,
            fromCurrency,
            BN(dy1).toFixed(0),
          ...
          //console.log(+dy)
          let bidrate =
            (dy * abis[pools[j]].coin_precisions[fromCurrency]) / dx1;
      ...
      bids = bids.filter(
        (b) =>
          b[0] > Math.max(Math.max(...bxs.map((xs) => Math.min(...xs))), 0.1)
      );
      asks = asks.filter(
        (a) =>
          a[0] < Math.min(Math.min(...axs.map((xs) => Math.max(...xs))), 10)
      );
      bids = bids.reverse();
      ...
      this.currentValue =
        (Math.max(...bxs.flat()) + Math.min(...axs.flat())) / 2;
    ...
    move(event) {
      ...
      if (this.chart.crossLabel) {
        ...
        this.chart.hoverPoint &&
          this.chart.crossYLabelLeft.attr({
            y: this.chart.hoverPoint.plotY + this.chart.plotTop + 25,
            text: this.chart.yAxis[0]
              .toValue(this.chart.hoverPoint.plotY + this.chart.plotTop)
              .toFixed(4),
          });
        ...
        if (
          !document
            .querySelector(
              "#depth_chart .highcharts-series.highcharts-series-1"
            )
            .classList.contains("highcharts-series-hover")
        )
