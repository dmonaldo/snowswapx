...
export default {
  ...
  methods: {
    async mounted() {
      ...
      let virtual_price = await contract.swap.methods
        .get_virtual_price()
        .call();
      ...
      for (let i = 0; i < 2; i++) {
        ...
        var rate =
          parseInt(
            await contract.coins[i].methods.exchangeRateStored().call()
          ) /
          1e18 /
          contract.coin_precisions[i];
        var supply_rate = parseInt(
          await contract.coins[i].methods.supplyRatePerBlock().call()
        );
        var old_block = parseInt(
          await contract.coins[i].methods.accrualBlockNumber().call()
        );
      ...
      for (let i = 1; i < 100; i++) {
        let volume = i;
        let dy =
          +calc.get_dy_underlying(0, 1, BN(dx1).times(i).toFixed(0), true) /
          1e6;
        let dx =
          +calc.get_dy_underlying(1, 0, BN(dy1).times(i).toFixed(0), true) /
          1e18;
        //console.log(+dy)
      ...
      let currentValue = +calc
        .get_dy_underlying(0, 1, BN(1e18).toFixed(0), true)
        .div(BN(1e6));
      this.$refs.highcharts.chart.series[0].xAxis.plotLinesAndBands[0].update({
        value: currentValue,
        label: {
    ...
    move(event) {
      ...
      var path = [
        ...
        x,
        "M",
        this.chart.plotWidth + 2 * this.chart.plotLeft - y,
        this.chart.plotTop,
        "L",
        this.chart.plotWidth + 2 * this.chart.plotLeft - y,
      ...
      var isOnPlot = () => true;
      console.log(
        this.chart.pointer.findNearestKDPoint(this.chart.series, false, {
          clientX: 0,
          clientY: 0,
        })
      );
      ...
      console.log(
        this.chart.pointer.findNearestKDPoint(this.chart.series, false, ne)
      );
      console.log(
        this.chart.pointer.findNearestKDPoint(this.chart.series, true, ne)
      );
      console.log(
        this.chart.pointer.findNearestKDPoint(this.chart.series, true, {
          ...ne,
          chartX: 1,
          chartY: 2,
        })
      ...
      this.chart.pointer
        .findNearestKDPoint(this.chart.series, false, {
          ...this.chart.series.normalize(event),
          chartX: 100,
          chartY: 100,
        })
        .select(true, false);
      ...
      if (this.chart.crossLines) {
        ...
        // draw lines
        this.chart.crossLines = this.chart.renderer
          .path(path)
          .attr({
            "stroke-width": 1,
            stroke: "green",
      ...
      if (this.chart.crossLabel) {
        if (isOnPlot(x, y, this.chart)) {
          ...
          this.chart.crossLabel.attr({
            ...
            text: this.chart.xAxis[0]
              .toValue(this.chart.plotWidth - y)
              .toFixed(4),
          });
          ...
          if (
            document
              .querySelector(".highcharts-series.highcharts-series-0")
              .getAttribute("opacity") == 1
          )
            this.chart.crossYLabelLeft.attr("y", -9999);
          else this.chart.crossYLabelRight.attr("y", -9999);
        } else {
        ...
        // draw label
        this.chart.crossLabel = this.chart.renderer
          .text(
            this.chart.xAxis[0].toValue(x).toFixed(4),
            x - 15,
            this.chart.plotTop - 5
          )
          .add();
        this.chart.crossYLabelLeft = this.chart.renderer
          .text(
            this.chart.yAxis[0].toValue(y).toFixed(4),
            this.chart.plotLeft - 5,
            y - 5
          )
          .attr({
            rotation: -90,
          ...
          .add();
        this.chart.crossYLabelRight = this.chart.renderer
          .text(
            this.chart.yAxis[0].toValue(y).toFixed(4),
            this.chart.plotLeft + this.chart.plotWidth + 15,
            y
          )
          .attr({
            rotation: -90,
