...
let calcPrice = (
  ...
  return calc
    .get_dy_underlying(fromCurrency, toCurrency, precisions, usefee)
    .toString(10);
};

let calcPriceWrapped = (
  config,
  fromCurrency,
  toCurrency,
  precisions,
    ...
    for (let pool of poolnames) {
      ...
      for (let [pair, val] of vol) {
        ...
        volumes[normM].push([
          v.timestamp * 1000,
          volSold /
            allabis[key == "y" ? "iearn" : key == "susd" ? "susdv2" : key]
              .coin_precisions[m],
        ]);
        volumes[normN].push([
          v.timestamp * 1000,
...
let pairToName = (pair, pool) => {
  ...
  return (
    Object.keys(currencies[pool])[i] +
    " ⇄ " +
    Object.keys(currencies[pool])[j]
  ).toUpperCase();
};
    ...
    for (let pool of poolnames) {
      ...
      for (let [pair, val] of vol) {
        ...
        volumes[pairName].push([
          v.timestamp * 1000,
          val[0] /
            allabis[key == "susd" ? "susdv2" : key].coin_precisions[
              pair.split("-")[0]
            ],
        ]);
...
Comlink.expose({
  calcPrice: calcPrice,
  calcPriceWrapped: calcPriceWrapped,
  getVolumePerCoin,
  getVolumePerPair,
});
