...
export default {
  ...
  methods: {
    async mounted() {
      ...
      if (params && params.length) {
        ...
        if (params.length == 3)
          this.pools = tradeStore.allPools.filter((pool) =>
            params[0].split("_").includes(pool)
          );
        this.pair = this.allPairs.find((p) => p.val == pair.toLowerCase());
        this.interval = interval;
        this.emitSelect();
      } else this.pair = this.pairs[0];
      //this.$emit('selected', this.pool, this.pair, this.interval)
    },
    emitSelect() {
      let currenciesPools = Object.values(this.filteredCurrencies).map((v) =>
        Object.keys(v)
      );
      let pairPools = this.isUnique(this.pair.val);
      if (pairPools.length == 1) {
        let pairPoolIndex = currenciesPools
          .map((v) => v.join())
          .indexOf(pairPools[0].join());
      ...
      let poolsWithPair = Object.fromEntries(
        Object.entries(this.filteredCurrencies).filter(
          ([key, pairs]) =>
            Object.keys(pairs).includes(currency_i) &&
            Object.keys(pairs).includes(currency_j)
        )
      );
      poolsWithPair = Object.keys(poolsWithPair).map((pool) =>
        pool == "iearn" ? "y" : pool
      );
      this.pools = tradeStore.pools = poolsWithPair.filter((pool) =>
        this.pools.includes(pool)
      );
  ...
  computed: {
    allPairs() {
      ...
      let pairs = Object.values(currencies)
        .filter((p) => p != "susd")
        .map((v) => Object.keys(v));
      let allPairs = [];
      for (let pair of pairs) {
        for (let [i, val] of pair.entries()) {
          for (let [j, val1] of pair.entries()) {
            if (
              i != j &&
              allPairs.find((p) => p.val == `${val}-${val1}`) === undefined
            ) {
              allPairs.push({
                idx: `${i}-${j}`,
                val: `${val}-${val1}`,
    ...
    pairs() {
      ...
      let duplicates;
      if (pools.length > 1) {
        let filteredCurrencies = Object.keys(
          this.filteredCurrencies
        ).filter((p) => pools.includes(p));
        duplicates = Object.keys(this.filteredCurrencies)
          .flatMap((f) => Object.keys(this.filteredCurrencies[f]))
          .filter(
            (k, i, all) => all.indexOf(k) === i && all.lastIndexOf(k) !== i
          );
      } else {
        duplicates = Object.keys(
          this.filteredCurrencies[
            this.pools[0] == "y" ? "iearn" : this.pools[0]
          ]
        );
      }
      var pairs = [];
    ...
    filteredCurrencies() {
      ...
      return Object.fromEntries(
        Object.entries(allCurrencies).filter(
