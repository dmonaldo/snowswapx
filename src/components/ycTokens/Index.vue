<template>
  <div class="window white">
    <div class="info-message gentle-message">
      ycTokens(
      <a
        href="https://etherscan.io/token/0x99d1fa417f94dcd62bfe781a1213c092a47041bc"
      >
        yDAI
      </a>
      ,
      <a
        href="https://etherscan.io/token/0x9777d7e2b60bb01759d0e2f8be2095df444cb07e"
      >
        yUSDC
      </a>
      ,
      <a
        href="https://etherscan.io/token/0x1be5d71f2da660bfdee8012ddc58d024448a0a59"
      >
        yUSDT
      </a>
      ) are forked from iearn's yTokens, without owner and with Compound lending
      available for ycUSDT
    </div>
      ...
      <div class="tokens">
        <div class="inputs">
          <div class="maxBalanceContainer" @click="setBalance(i, 0)">
            ...
            <sub class="maxBalance">
              Bal:
              {{ (token.maxBalance / token.precisions).toFixed(2) }}
              ({{
                ((token.maxBalance / token.precisions) * token.rate).toFixed(2)
              }}
...
export default {
  ...
  methods: {
    async mounted() {
      let iearnAPR = new contract.web3.eth.Contract(
        iearnAPR_abi,
        iearnAPR_address
      );
      let i = 0;
      ...
      let calls = this.ycTokens.flatMap((t) => [
        [
          t.ucontract._address,
          t.ucontract.methods.balanceOf(contract.default_account).encodeABI(),
        ],
        [
          t.contract._address,
          t.contract.methods.balanceOf(contract.default_account).encodeABI(),
        ],
        [
          iearnAPR_address,
          iearnAPR.methods.getAPROptions(t.ucontract._address).encodeABI(),
        ],
        [
          t.contract._address,
          t.contract.methods.getPricePerFullShare().encodeABI(),
        ],
      ]);
      ...
      let balances = aggcalls[1].map((hex, i) => {
        ...
        if (i % 4 == 2) {
          return Object.values(
            contract.web3.eth.abi.decodeParameters(paramArray, hex)
          );
        }
        return contract.web3.eth.abi.decodeParameter("uint256", hex);
      });
      chunkArr(balances, 4).map(
        ([ubal, bal, [, capr, , , , aapr, , dapr], rate], i) => {
          ...
          Vue.set(
            this.ycTokens,
            i,
            Object.assign({}, this.ycTokens[i], { maxBalanceUnderlying: ubal })
          );
        }
      );
    },
    setBalance(i, n) {
      ...
      if (n == 0) {
        Vue.set(
          this.ycTokens,
          i,
          Object.assign({}, this.ycTokens[i], {
            value: (
              this.ycTokens[i].maxBalance / this.ycTokens[i].precisions
            ).toFixed(2),
          })
        );
      }
      if (n == 1) {
        Vue.set(
          ...
          Object.assign({}, this.ycTokens[i], {
            uvalue: (
              this.ycTokens[i].maxBalanceUnderlying /
              this.ycTokens[i].precisions
            ).toFixed(2),
          })
        );
      }
    ...
    changeUValue(i) {
      if (
        this.ycTokens[i].uvalue >
        this.ycTokens[i].maxBalanceUnderlying / this.ycTokens[i].precisions
      ) {
        Vue.set(this.ubgColors, i, "red");
      } else Vue.set(this.ubgColors, i, "blue");
    },
    async deposit(i) {
      let amount = BN(this.ycTokens[i].uvalue).times(
        this.ycTokens[i].precisions
      );
      await approveAmount(
        this.ycTokens[i].ucontract,
        amount,
        contract.default_account,
        this.ycTokens[i].contract._address
      );
      await this.ycTokens[i].contract.methods
        .deposit(amount.toFixed(0, 1))
        .send({
    ...
    async withdraw(i) {
      ...
      let amount = BN(this.ycTokens[i].value)
        .times(this.ycTokens[i].precisions)
        .toFixed(0, 1);
