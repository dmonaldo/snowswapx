<template>
  <div>
    ...
    <div class="window white">
      <highcharts
        :constructor-type="'stockChart'"
        :options="liquidityutilizationdata"
...
export default {
          ...
          series: {
            ...
            point: {
              events: {
                click: (function (self) {
                  return function () {
                    let index = this.dataGroup
                      ? this.dataGroup.start
                      : this.index;
  ...
  created() {
    this.$watch(
      () => contract.multicall && contract.default_account,
      (val, oldval) => {
  ...
  async mounted() {
    ...
    let pools = Object.keys(allabis).filter(
      (pool) =>
        pool != "susd" && pool != "y" && pool != "tbtc" && pool != "hbtc"
    );
    await volumeStore.fetchVolumeData(pools, true, 1440);
    let data = volumeStore.state.volumeData[1440];
    let btcPrices = await helpers.retry(
      fetch(
        `https://api.coingecko.com/api/v3/coins/bitcoin/market_chart/range?vs_currency=usd&from=1589587198&to=${
          (Date.now() / 1000) | 0
        }`
      )
    );
    ...
    for (let btcPool of ["ren", "sbtc", "hbtc"]) {
      data[btcPool] = data[btcPool].map((d) => {
        d.balances = d.balances.map(
          (bal) =>
            bal * volumeStore.findClosestPrice(d.timestamp, btcPrices.prices)
        );
      ...
      for (let point of points) {
        ...
        let coinBalances = point.balances.map((bal, i) => {
          return (
            (bal * point.rates[i]) /
            1e18 /
            allabis[pool == "susd" ? "susdv2" : pool].coin_precisions[i]
          );
        });
        let volumeBalances = Object.entries(point.volume).map(([k, vol]) => {
          return (
            vol[0] /
            allabis[pool == "susd" ? "susdv2" : pool].coin_precisions[
              k.split("-")[0]
            ]
          );
        });
        ...
        volumes[pool].push([point.timestamp * 1000, totalBalances]);
        volumeperpool[pool].push([point.timestamp * 1000, totalVolume]);
    ...
    for (let [pool, liquidityUtilization] of Object.entries(
      liquidityUtilizations
    )) {
      if (["tbtc", "hbtc"].includes(pool)) continue;
    ...
    for (let [pool, points] of Object.entries(data)) {
      ...
      for (let point of points) {
        ...
        for (let [i, balance] of coinBalances.entries()) {
          ...
          let hasPoint = coinbalances[coins[i]].find(
            (p) => p[0] == point.timestamp * 1000
          );
          if (hasPoint !== undefined) {
            hasPoint[1] += balance;
          } else coinbalances[coins[i]].push([point.timestamp * 1000, balance]);
        }
      }
    }
    ...
    let balances = Object.keys(volumes).reduce((obj, key) => {
      return {
        ...obj,
        [key]: new Array(
          Math.max(...Object.values(volumes).map((arr) => arr.length)) -
            volumes[key].length
        )
          .fill({})
          .concat(volumes[key]),
    ...
    this.allPools = Object.fromEntries(
      Object.entries(volumes)
        .filter(([p, v]) => !["tbtc", "hbtc"].includes(p))
        .map(([p, v]) => [p, volumes[p][volumes[p].length - 1][1]])
    );

    let latestDeposits = Object.keys(volumes)
      .filter((p) => !["tbtc", "hbtc"].includes(p))
      .map((p) => volumes[p][volumes[p].length - 1][1]);

    let poolHoldings = latestDeposits.reduce((a, b) => a + b, 0);

    let poolPercentages = latestDeposits.map(
      (poolDeposits) => (poolDeposits / poolHoldings) * 100
    );

    poolPercentages = poolPercentages.map((percentage, i) => ({
      name: Object.keys(volumes).filter((p) => !["tbtc", "hbtc"].includes(p))[
        i
      ],
      y: poolPercentages[i],
    }));

    let highest = poolPercentages
      .map((data) => data.y)
      .indexOf(Math.max(...poolPercentages.map((data) => data.y)));
    ...
    this.allCoins = Object.fromEntries(
      Object.entries(coinbalances)
        .filter(([p, v]) => !["tbtc", "hBTC"].includes(p))
        .map(([p, v]) => {
    ...
    latestDeposits = Object.keys(coinbalances)
      .filter((p) => !["tbtc", "hbtc"].includes(p.toLowerCase()))
      .map((p) => coinbalances[p][coinbalances[p].length - 1][1]);

    poolHoldings = latestDeposits.reduce((a, b) => a + b, 0);

    poolPercentages = latestDeposits.map(
      (poolDeposits) => (poolDeposits / poolHoldings) * 100
    );

    poolPercentages = poolPercentages.map((percentage, i) => ({
      name: Object.keys(coinbalances).filter(
        (p) => !["hbtc", "tbtc"].includes(p)
      )[i],
      y: poolPercentages[i],
    }));

    highest = poolPercentages
      .map((data) => data.y)
      .indexOf(Math.max(...poolPercentages.map((data) => data.y)));
    poolPercentages[highest].sliced = true;
    poolPercentages[highest].selected = true;
    ...
    this.chart.update({
      title: {
        text: `Total USD Deposits ${helpers.formatNumber(
  ...
  methods: {
    async showBalances() {
      ...
      helpers
        ...
        .map((v, i) => {
          ...
          if (["tbtc", "ren", "sbtc", "hbtc"].includes(key))
            Vue.set(this.balances, key, this.balances[key] * btcPrice);
        });
      ...
      Vue.set(
        ...
        "sbtc",
        this.balances.sbtc +
          ((+decoded[len - 2] * decoded[15]) / 1e36) * btcPrice
      );
      Vue.set(
        this.balances,
        "y",
      ...
      let deposits = Object.fromEntries(
        Object.entries(this.balances).map(([k, v]) => [k, v > 0 ? v : 0])
      );
      ...
      if (this.totalDeposits > 0) {
        ...
        let highest = depositPercentages
          .map((data) => data.y)
          .indexOf(Math.max(...depositPercentages.map((data) => data.y)));
