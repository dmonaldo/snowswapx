<template>
  <div>
    ...
    <div
      class="window white"
      v-for="(currency, i) in Object.keys(filteredCurrencies)"
    >
      <p class="simple-error" v-show="currency == 'susd'">
        Old susd pool. Please
        <router-link to="/susd/withdraw">withdraw and move</router-link>
        funds to
        <router-link to="/susdv2">susdv2</router-link>
        pool
      </p>
      <p class="text-center">
        <router-link
          :to="currency"
          v-show="
            currency != 'susd' && !['tbtc', 'ren', 'sbtc'].includes(currency)
          "
        >
          {{ currency == "iearn" ? "y" : currency }}.snowswap.org
        </router-link>
        <span v-show="['tbtc', 'ren', 'sbtc'].includes(currency)">
          {{ currency }}
        </span>
        <a href="https://iearn.finance/pool" v-show="currency == 'susd'">
          susd
        </a>
      </p>
      <stats :pool="currency" />
      <balances-info
        ...
        :staked_info="
          ['susdv2', 'sbtc', 'y', 'iearn'].includes(currency) &&
          staked_infos[currency]
        "
...
export default {
  metaInfo: {
    title: "snowswap.org - Stats",
    meta: [
      { property: "og:url", content: "https://snowswap.org/combinedstats" },
      { name: "twitter:url", content: "https://snowswap.org/combinedstats" },
    ],
  },
  components: {
    Stats,
  ...
  computed: {
    ...
    filteredCurrencies() {
      return Object.fromEntries(
        Object.entries(this.allCurrencies).filter(
  ...
  methods: {
    ...
    async init_contracts() {
      for (let [key, contract] of Object.entries(this.allContracts)) {
        ...
        this.web3contracts[key].swap = new currentContract.web3.eth.Contract(
          contract.swap_abi,
          contract.swap_address
        );
        this.web3contracts[
          key
        ].swap_token = new currentContract.web3.eth.Contract(
          ERC20_abi,
        ...
        for (let i = 0; i < contract.N_COINS; i++) {
          ...
          let cabi = ["iearn", "y", "busd", "susd", "pax"].includes(key)
            ? yERC20_abi
            : cERC20_abi;
          if (key == "susd" && i == 1) {
          ...
          this.all_coins[key].coins[i] = new currentContract.web3.eth.Contract(
            cabi,
            addr
          );
          ...
          this.all_underlying_coins[key].underlying_coins[
            i
          ] = new currentContract.web3.eth.Contract(ERC20_abi, underlying_addr);
        }
      }
    },
      ...
      for (let [key, contract] of Object.entries(this.allContracts)) {
        ...
        for (let i = 0; i < contract.N_COINS; i++) {
          ...
          if (
            (contract.tethered &&
              contract.tethered[i] &&
              contract.use_lending &&
              !contract.use_lending[i]) ||
            key == "susdv2" ||
            (key == "pax" && i == 3) ||
            key == "tbtc" ||
            key == "ren" ||
            ...
            if (
              ...
              calls.push([
                ...
                this.all_coins[key].coins[i].methods
        ...
        calls.push(
          [
            this.web3contracts[key].swap._address,
            this.web3contracts[key].swap.methods
              .get_virtual_price()
              .encodeABI(),
          ],
          [
          ...
          [
            ...
            this.web3contracts[key].swap_token.methods
              .balanceOf(
    ...
    async update_fee_info(version = "new") {
      ...
      calls.push([
        contracts.susdv2.sCurveRewards_address,
      ...
      calls.push([
        contracts.sbtc.sCurveRewards_address,
        "0x70a08231000000000000000000000000" +
          (
            currentContract.default_account ||
            "0x0000000000000000000000000000000000000000"
      ...
      calls.push([
        contracts.iearn.sCurveRewards_address,
        "0x70a08231000000000000000000000000" +
          (
            currentContract.default_account ||
            "0x0000000000000000000000000000000000000000"
      ...
      for (let [key, contract] of Object.entries(this.allContracts)) {
        ...
        if (key == "usdt") {
          this.bal_infos.usdt.push(
            this.all_c_rates["compound"].c_rates[0] * +decoded[ind]
          );
          this.bal_infos.usdt.push(
            this.all_c_rates["compound"].c_rates[1] * +decoded[ind + 1]
          );
          this.all_c_rates[key].c_rates[2] =
            1 / contracts[key].coin_precisions[2];
          let calcBalance =
            +decoded[ind + 2] * this.all_c_rates[key].c_rates[2];
          ind -= 5;
          this.bal_infos[key].push(calcBalance);
          total +=
            this.bal_infos.usdt[0] + this.bal_infos.usdt[1] + calcBalance;
        }
        if (
          key == "iearn" ||
          key == "busd" ||
        ...
        for (let i = 0; i < contracts[key].N_COINS; i++) {
          var val =
            (this.bal_infos[key][i] * +decoded[ind + 14]) / +decoded[ind + 15];
        ...
        if (["susdv2", "sbtc", "y", "iearn"].includes(key)) {
          ...
          if (stakedBalance > 0) {
            for (let i = 0; i < contracts[key].N_COINS; i++) {
              var val =
                (this.bal_infos[key][i] * stakedBalance) / +decoded[ind + 15];
