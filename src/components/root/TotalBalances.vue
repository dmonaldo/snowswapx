<template>
  <div class="window gray-block">
    <div>
      <h3>Total pool deposits and daily volume</h3>
      <ul>
        <li id="total-balances">
          <b>Deposits: </b>
          <span v-show="total">{{ total | formatNumber }}$</span>
        </li>
        <li>
          <b>Daily volume: </b>
          <span v-show="volume >= 0">{{ volume | 0 | formatNumber(0) }}$</span>
        </li>
      </ul>
      <slot></slot>
    </div>
...
export default {
  ...
  computed: {
    volume() {
      return this.totalVolume !== undefined
        ? this.totalVolume
        : volumeStore.totalVolume();
    },
  },
  async created() {
    this.totalBalances();
    if (this.totalVolume === undefined) this.dailyVolume();
  },
  methods: {
    async totalBalances() {
      ...
      for (let [key, contract] of Object.entries(pools)) {
        ...
        tokenContracts[key] = new web3.eth.Contract(
          ERC20_abi,
          contract.token_address
        );
        swapContracts[key] = new web3.eth.Contract(
          contract.swap_abi,
          contract.swap_address
        );
        calls.push([
          tokenContracts[key]._address,
          tokenContracts[key].methods.totalSupply().encodeABI(),
        ]);
        calls.push([
          swapContracts[key]._address,
          swapContracts[key].methods.get_virtual_price().encodeABI(),
        ]);
      }
      let susd_swap_token = new web3.eth.Contract(
        ERC20_abi,
        allabis.susd.token_address
      );
      let susd_swap = new web3.eth.Contract(
        allabis.susd.swap_abi,
        allabis.susd.swap_address
      );
      ...
      chunkArr(decoded, 2).map((v, i, arr) => {
        ...
        //renBTC
        if ([6, 7, 8].includes(i))
          balance = balance.times(BN(priceStore.state.btcPrice));
