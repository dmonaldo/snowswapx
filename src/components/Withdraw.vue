  ...
  <div class="window white add-liquidity">
    <fieldset class="percentage">
      <legend>
        ...
        <input
          ...
          v-model="showstaked"
        />
        <label for="showstaked" v-show="currentPool == 'susdv2'">
          Show staked
        </label>
      </legend>
      <ul>
        ...
        <li v-for="(currency, i) in Object.keys(currencies)">
          <label :for="'currency_' + i" class="currency_label">
            ...
            <span
              v-show="
                !withdrawc &&
                !['susdv2', 'tbtc', 'ren', 'hbtc'].includes(currentPool)
              "
            >
              {{ currency | capitalize }}
            </span>
    ...
    <div id="withdraw_buttons">
      ...
      <button
        id="remove-liquidity"
        :disabled="
          (currentPool == 'susdv2' && slippage < -0.03 && !warninglow) ||
          show_nobalance == true
        "
      ...
      <router-link
        v-show="currentPool == 'susdv2' && oldBalance > 0"
        class="button"
        to="/susd/withdraw"
        id="withdrawold"
      >
      ...
      <div class="info-message gentle-message" v-show="estimateGas">
        Estimated tx cost:
        {{ (((estimateGas * gasPrice) / 1e18) * ethPrice).toFixed(2) }}$
      </div>
...
export default {
  ...
  computed: {
    ...
    minAmount() {
      if (["tbtc", "ren", "hbtc"].includes(currentContract.currentContract))
        return 1e-8;
      return 0.01;
    },
    calcFee() {
  ...
  methods: {
    ...
    toFixed(num, precisions = 2, round = 4) {
      if (
        precisions == 2 &&
        ["tbtc", "ren", "hbtc"].includes(currentContract.currentContract)
      )
        precisions = 8;
    ...
    async update_balances() {
      ...
      calls.push([
        currentContract.curveGauge._address,
        currentContract.curveGauge.methods
          .balanceOf(
            currentContract.default_account ||
              "0x0000000000000000000000000000000000000000"
          )
    ...
    async handle_change_amounts(i, event) {
      ...
      try {
        ...
        if (
          availableAmount.gt(maxAvailableAmount.plus(BN(this.staked_balance)))
        ) {
    ...
    async unstake(amount, exit = false) {
      this.waitingMessage = `
                    Need to unstake ${amount
                      .div(BN(1e18))
                      .toFixed(0, 1)} tokens from Mintr for withdrawal.
                    <br>
    ...
    async handle_remove_liquidity(unstake = false) {
      ...
      let promises = await Promise.all([
        helpers.getETHPrice(),
        currentContract.web3.eth.getGasPrice(),
      ]);
      ...
      if (this.share == "---") {
        ...
        if (
          (this.token_balance.lt(amount) || unstake) &&
          this.currentPool == "susdv2"
        )
        ...
        if (this.to_currency !== null && this.to_currency < 10) {
          ...
          try {
            ...
            await currentContract.swap.methods
              .remove_liquidity(amount, min_amounts)
              .send({ from: currentContract.default_account, gas: 600000 })
              .once(
                "transactionHash",
                () =>
                  (this.waitingMessage =
